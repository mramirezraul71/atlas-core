...
from modules.evolution.credentials import (ensure_pypi_password, get_pypi_auth,
                                           load_credentials)
...
DASHBOARD_URL = (
    os.environ.get("ATLAS_DASHBOARD_URL") or "http://127.0.0.1:8791"
).rstrip("/")
...
GITHUB_REPOS = [
    ...
    for r in os.environ.get(
        "ATLAS_EVOLUTION_GITHUB_REPOS", "mramirezraul71/atlas-core"
    ).split(",")
    if r.strip()
]
...
EVOLUTION_GOVERNED = os.environ.get("EVOLUTION_GOVERNED", "").strip().upper() in (
    "1",
    "TRUE",
    "GOVERNED",
)
...
async def _ping_nexus_and_log() -> None:
    ...
    try:
        ...
        url = (os.environ.get("NEXUS_BASE_URL") or "http://127.0.0.1:8000").rstrip(
            "/"
        ) + "/health"
        ...
        ok = False
    msg = (
        "[CONEXIÓN] Buscando NEXUS en puerto 8000... OK."
        if ok
        else "[CONEXIÓN] Buscando NEXUS en puerto 8000... Desconectado."
    )
...
class AtlasEvolutionDaemon:
    ...
    def __init__(self) -> None:
        ...
        if self._governed:
            logger.info(
                "Modo GOVERNED: los cambios al núcleo requieren confirmación en el Dashboard"
            )
        if not self._github_enabled:
            logger.warning(
                "Worker GitHub deshabilitado: falta github_token en credenciales"
            )
        if not self._hf_enabled:
            logger.warning(
                "Worker Hugging Face deshabilitado: falta hf_token en credenciales"
            )
    ...
    async def _worker_pypi(self) -> None:
        ...
        try:
            await _bitacora_log(
                "[EVOLUCIÓN] Fase PyPI (Software): escaneo de requirements.txt"
            )
            if not REQUIREMENTS_TXT.exists():
                logger.info("[PyPI] requirements.txt no encontrado, omitiendo")
                await _bitacora_log(
                    "[EVOLUCIÓN] requirements.txt no encontrado. Omitiendo fase PyPI.",
                    ok=True,
                )
                return
            lines = REQUIREMENTS_TXT.read_text(
                encoding="utf-8", errors="ignore"
            ).splitlines()
            entries = [_parse_requirements_line(l) for l in lines]
            packages_to_scan = [
                (n, cur_ver, orig) for n, cur_ver, orig in entries if n
            ][:8]
            if not packages_to_scan:
                await _bitacora_log(
                    "[EVOLUCIÓN] Sin paquetes a escanear en requirements.txt", ok=True
                )
                return
            import base64
            import urllib.request
            ...
            if pypi_user and pypi_pass:
                auth_header = base64.b64encode(
                    f"{pypi_user}:{pypi_pass}".encode()
                ).decode()
            venv_py = (
                TEMP_VENV / "Scripts" / "python.exe"
                if os.name == "nt"
                else TEMP_VENV / "bin" / "python"
            )
            pip = (
                TEMP_VENV / "Scripts" / "pip.exe"
                if os.name == "nt"
                else TEMP_VENV / "bin" / "pip"
            )
            if not venv_py.exists():
                await _bitacora_log(
                    "[SANDBOX] Iniciando entorno de pruebas aislado (.temp_venv)..."
                )
                subprocess.run(
            ...
            if not pip.exists():
                await _bitacora_log(
                    "[ERROR] Fallo en Sandbox: no se pudo crear pip. Reintentando en próximo ciclo (12h).",
                    ok=False,
                )
                return
            env = dict(os.environ)
            if pypi_user and pypi_pass:
                ...
                pypirc.write_text(
                    "[pypi]\nusername = __token__\npassword = "
                    + ensure_pypi_password(self._creds.get("pypi_token", "")),
                    encoding="utf-8",
                )
            ...
            for pkg_name, current_ver, original_line in packages_to_scan:
                ...
                if r.returncode != 0:
                    await _bitacora_log(
                        "[ERROR] Fallo en Sandbox. Reintentando en próximo ciclo (12h).",
                        ok=False,
                    )
                ...
                imp = subprocess.run(
                    [str(venv_py), "-c", f"import {import_module}"],
                    capture_output=True,
                    timeout=15,
                    cwd=str(BASE),
                )
                if imp.returncode != 0:
                    ...
                    continue
                await _bitacora_log(
                    "[OK] Test de integridad superado. Asimilando herramienta..."
                )
                ...
                if self._governed:
                    ...
                    await _bitacora_log(
                        "[EVOLUCIÓN] Modo GOVERNED: pausa. Aprobar Asimilación en el Dashboard (sección Aprobaciones).",
                        ok=True,
                    )
                    logger.info(
                        "[PyPI] Sandbox OK (GOVERNED): %s %s -> %s — pendiente de confirmación en Dashboard",
                        pkg_name,
                        current_ver or "?",
                        latest_ver,
                    )
                ...
                await _bitacora_log(
                    f"[OK] Asimilación autónoma (GROWTH): {pkg_name}=={latest_ver}. Núcleo actualizado.",
                    ok=True,
                )
                ...
                for file_line in lines:
                    ...
                    if n == pkg_name and parsed_orig == original_line:
                        ...
                        new_content.append(
                            file_line if file_line.endswith("\n") else file_line + "\n"
                        )
                ...
                logger.info(
                    "[PyPI] Asimilación de Mejora Exitosa: %s %s -> %s; requirements.txt actualizado",
                    pkg_name,
                    current_ver or "?",
                    latest_ver,
                )
                ...
                await _bitacora_log(
                    "[PYPI] Analizando requirements.txt... 0 cambios pendientes.",
                    ok=True,
                )
                await _notify_dashboard(
                    {
    ...
    async def _worker_github(self) -> None:
        ...
        try:
            await _bitacora_log(
                "[EVOLUCIÓN] Fase GitHub (Código): sincronización de repositorios Rauli y Trading"
            )
            import urllib.error
            import urllib.request
            ...
            for repo in GITHUB_REPOS[:3]:
                ...
                repo_label = (
                    "RAULI_CORE"
                    if "rauli" in repo.lower() or "Rauli" in repo
                    else repo.split("/")[-1].upper().replace("-", "_")
                )
                await _bitacora_log(
                    f"[GH] Sincronizando repositorio {repo_label}... Actualizado.",
                    ok=True,
                )
                await _notify_dashboard(payload)
                logger.info(
                    "[GitHub] %s tag=%s commits=%s tools=%s",
                    repo,
                    release_tag,
                    len(commits_info),
                    tools,
            ...
            await _bitacora_log(
                "[ERROR] Fase GitHub fallida. Reintentando en próximo ciclo (12h).",
                ok=False,
            )
            await _notify_dashboard(
                {"worker": "github", "status": "error", "message": str(e)[:200]}
            )
    ...
    async def _worker_huggingface(self) -> None:
        ...
        try:
            await _bitacora_log(
                "[EVOLUCIÓN] Fase Hugging Face (IA): búsqueda de modelos para Insta360 Link 2"
            )
            import urllib.parse
            import urllib.request
            ...
            collected = []
            for query in (
                HF_SEARCH_QUERIES or ["vision", "language model", "small", "ocr"]
            )[:4]:
                try:
                    url = (
                    ...
                    collected.append(
                        {
                            "id": model_id,
                            "query": query,
                            "meta": m if isinstance(m, dict) else {"id": model_id},
                        }
                    )
            for item in collected[:3]:
                model_id = item["id"]
                cache_file = (
                    TEMP_MODELS_CACHE / f"{model_id.replace('/', '_')}.meta.json"
                )
            ...
            if not collected:
                await _bitacora_log(
                    "[EVOLUCIÓN] Hugging Face: sin modelos nuevos en esta pasada. Cache OK.",
                    ok=True,
                )
                await _notify_dashboard(
                    {
            ...
            await _bitacora_log(
                "[ERROR] Fase Hugging Face fallida. Reintentando en próximo ciclo (12h).",
                ok=False,
            )
            await _notify_dashboard(
                {"worker": "huggingface", "status": "error", "message": str(e)[:200]}
            )
        ...
        await _notify_dashboard(
            {
                ...
                "workers": {
                    "pypi": self._pypi_enabled,
                    "github": self._github_enabled,
                    "huggingface": self._hf_enabled,
                },
            }
        )
                ...
                json.dumps(
                    {
                        ...
                        "message": "Asimilación Exitosa"
                        if cycle_ok
                        else "Ciclo con errores",
                    }
                ),
                encoding="utf-8",
            )
    ...
    async def run_forever(self) -> None:
        ...
        logger.info(
            "ATLAS_EVOLUTION daemon iniciado — ciclo cada %ss (%.1fh)",
            self._interval,
            self._interval / 3600,
        )
        while True:
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(
        description="ATLAS_EVOLUTION — Tríada PyPI/GitHub/Hugging Face"
    )
    parser.add_argument(
        "--run-once",
        action="store_true",
        help="Ejecutar un solo ciclo y salir (para forzar actualización vía API)",
    )
