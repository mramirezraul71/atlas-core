...
NEXUS_HEALTH_URL = (os.environ.get("NEXUS_BASE_URL") or "http://127.0.0.1:8000").rstrip(
    "/"
) + "/health"
...
def _ping_nexus() -> bool:
    ...
    try:
        req = urllib.request.Request(
            NEXUS_HEALTH_URL, method="GET", headers={"Accept": "application/json"}
        )
...
def wait_for_nexus(timeout_sec: int = NEXUS_WAIT_TIMEOUT_SEC) -> bool:
    ...
    logger.error(
        "CEREBRO — CUERPO (NEXUS): Desconectado tras %ss. El robot no puede operar sin el cuerpo. Ejecuta C:\\ATLAS_NEXUS\\scripts\\start_all.ps1",
        timeout_sec,
    )
    return False
...
def main() -> None:
    ...
    if os.environ.get("EVOLUTION_CREDENTIALS_WARN") == "1":
        logger.info(
            "Evolution: proceso crítico registrado en ANS (evolution_health). Daemon: python atlas_evolution.py"
        )
