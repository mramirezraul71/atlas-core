"""
backtest/internal_gbm_simulator.py — Atlas Code Quant Internal GBM Simulator v1.0

.. warning::

    **ESTO NO ES LEAN / QUANTCONNECT REAL.**

    Este módulo es un **simulador GBM sintético interno** que combina
    Geometric Brownian Motion + régimen Markov para producir datasets de
    entrenamiento. No invoca runtime LEAN, no depende de QuantConnect, ni se
    conecta a un motor externo.

    Historia del renombre:
        * Antes de F2 vivía en ``atlas_code_quant/backtest/lean_simulator.py``
          y la clase principal se llamaba ``LeanSimulator``.
        * F2 (este commit) renombra el fichero a ``internal_gbm_simulator.py``
          y añade alias canónico ``InternalGBMSimulator = LeanSimulator``.
        * ``atlas_code_quant/backtest/lean_simulator.py`` queda como wrapper
          deprecated que reexporta y emite ``DeprecationWarning``.
        * El nombre interno de la clase (``LeanSimulator``) se conserva por
          compatibilidad. No se toca matemática ni firmas en F2.

    Para integración con LEAN/QuantConnect REAL ver ``atlas_code_quant/lean/``
    (scaffold F1, decisión arquitectónica en F9).
...
class LeanSimulator:  # noqa: D101  (rename interno planificado tras F9; véase alias `InternalGBMSimulator`)
...
# ── Alias canónico F2 ─────────────────────────────────────────────────────────
#
# Atlas Code Quant publica el simulador interno bajo el nombre honesto
# `InternalGBMSimulator`. La clase interna sigue llamándose `LeanSimulator`
# por compatibilidad con código existente (pattern_lab, scripts, tests),
# y para no introducir refactor funcional en F2.
#
# Referencias:
#   * docs/ATLAS_CODE_QUANT_F2_INTERNAL_SIMULATOR_RENAME.md
#   * atlas_code_quant/config/legacy_flags.py (LEAN_SIMULATOR_IS_INTERNAL_GBM)
InternalGBMSimulator = LeanSimulator


__all__ = [
    # Nombre canónico nuevo (F2)
    "InternalGBMSimulator",
    # Nombre histórico (compat)
    "LeanSimulator",
    # Configuración y record públicos
    "SimConfig",
    "TradeRecord",
]
