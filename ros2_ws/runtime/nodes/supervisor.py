...
class SupervisorNode(Node):
    def __init__(
        self, push_url="http://127.0.0.1:8791", robot_url="http://127.0.0.1:8002"
    ):
        ...
        self.create_subscription(
            None, "/atlas/perception/detections", self._perc_cb, 10
        )
        ...
        self.get_logger().info(
            f"Supervisor started: PUSH={push_url}, ROBOT={robot_url}"
        )
    ...
    def _heartbeat(self):
        ...
        if not self._push_ok:
            self.alert_pub.publish(
                make_string(json.dumps({"level": "warn", "msg": "PUSH unreachable"}))
            )
    ...
    def _health_check(self):
        ...
        if self._robot_ok:
            score += 15.0
        if self._subsystems.get("perception", {}).get("active"):
            score += 5.0
        if self._subsystems.get("slam", {}).get("active"):
            score += 5.0
        try:
        ...
        self.state_pub.publish(
            make_string(
                json.dumps(
                    {
                        "health": round(self._health_score, 1),
                        "push": self._push_ok,
                        "robot": self._robot_ok,
                        "governance": self.governance_mode,
                    }
    ...
    def _sync_governance(self):
        try:
            with urllib.request.urlopen(
                f"{self.push_url}/governance/status", timeout=3
            ) as r:
                d = json.loads(r.read().decode())
                ...
                if new and new != self.governance_mode:
                    ...
                    self.gov_pub.publish(
                        make_string(json.dumps({"mode": new, "prev": old}))
                    )
        except Exception:
            pass
...
class BrainBridgeNode(Node):
    ...
    def _query_cb(self, msg):
        ...
        threading.Thread(
            target=self._process, args=(qid, query, model), daemon=True
        ).start()
    ...
    def _process(self, qid, query, model):
        ...
        try:
            body = json.dumps(
                {"goal": query, "mode": "plan_only", "depth": 1, "fast": True}
            ).encode()
                        ...
                        json.dumps(
                            {
                                ...
                                "success": True,
                                "ms": ms,
                                "response": (result.get("data") or {}).get(
                                    "output", ""
                                ),
                                "model_used": (result.get("data") or {}).get(
                                    "model_used", ""
                                ),
                            }
                        )
