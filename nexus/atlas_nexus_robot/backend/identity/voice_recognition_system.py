...
class VoiceRecognitionSystem:
        ...
        try:
            if os.path.exists(self.voice_embeddings_file):
                with open(self.voice_embeddings_file, "rb") as f:
                    ...
                    logger.info(
                        f"✅ Loaded {len(self.owner_embeddings)} voice embeddings"
                    )
            else:
    ...
    def extract_voice_embedding(self, audio: np.ndarray) -> Optional[np.ndarray]:
        ...
        try:
            ...
            mfccs = librosa.feature.mfcc(
                y=audio, sr=self.sample_rate, n_mfcc=13, n_fft=2048, hop_length=512
            )
    ...
    def register_voice_from_audio(self, audio_data: str) -> Dict:
        ...
        try:
            ...
            if audio is None:
                return {"success": False, "message": "Failed to decode audio data"}
    ...
    def verify_speaker(self, audio_data: str) -> Dict:
        ...
        try:
            ...
            for registered_embedding in self.owner_embeddings:
                ...
                registered_norm = registered_embedding / np.linalg.norm(
            ...
            return {"is_owner": False, "confidence": 0.0, "message": f"Error: {str(e)}"}
    ...
    def analyze_voice_characteristics(self, audio_data: str) -> Dict:
        ...
        try:
            ...
            audio = self.decode_base64_audio(audio_data)
            if audio is None:
                return {"success": False, "message": "Failed to decode audio"}
            ...
            characteristics["zero_crossing_rate"] = float(
                np.mean(librosa.feature.zero_crossing_rate(audio)[0])
            )

            # Spectral centroid (brightness)
            characteristics["spectral_centroid"] = float(
                np.mean(librosa.feature.spectral_centroid(audio)[0])
            )
