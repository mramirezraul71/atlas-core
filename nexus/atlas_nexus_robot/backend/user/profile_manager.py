...
class ProfileManager:
    ...
    def get_user_profile(self, user_id: Optional[int] = None) -> Dict:
        ...
        try:
            ...
            if user_id:
                ...
                user = (
                    db.query(UserProfile)
                    .filter(UserProfile.id == self.current_user.id)
                    .first()
                )
            else:
    ...
    def update_user_profile(self, updates: Dict) -> Dict:
        ...
        try:
            if not self.current_user:
                return {"success": False, "message": "No current user set"}
    ...
    def get_interaction_history(
        self, limit: int = 50, interaction_type: Optional[str] = None
    ) -> Dict:
    ...
    def update_preference(
        ...
        try:
            ...
            preference = (
                ...
                .filter(
                    UserPreference.category == category, UserPreference.item == item
                )
                .first()
            )
            ...
            return {"success": False, "message": f"Error updating preference: {str(e)}"}
        finally:
            db.close()
    ...
    def get_preferences(self, category: Optional[str] = None) -> Dict:
        ...
        try:
            ...
            query = db.query(UserPreference).order_by(
