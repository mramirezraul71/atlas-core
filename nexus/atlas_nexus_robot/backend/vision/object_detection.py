...
class ObjectDetector:
    ...
    def load_model(self) -> bool:
        ...
        if YOLO is None:
            logger.warning(
                "YOLO deshabilitado: falta ultralytics. Instala 'ultralytics' para detección."
            )
            self.is_loaded = False
            self.model = None
            return False
    ...
    def draw_detections(self, frame: np.ndarray, detections: List[Dict]) -> np.ndarray:
        ...
        for det in detections:
            ...
            # Texto
            cv2.putText(frame_draw, label, (x1, y1 - 5), 0, 0.5, (255, 255, 255), 2)
    ...
    def get_summary(self, detections: List[Dict]) -> Dict:
        ...
        if not detections:
            return {"total": 0, "by_class": {}, "highest_confidence": None}
