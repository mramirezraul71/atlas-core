...
class YOLODetector:
    ...
    def __init__(
        ...
        if YOLO is None:
            logger.warning(
                "YOLO not available (ultralytics missing). Detection disabled."
            )
    ...
    def draw_detections(self, image: np.ndarray, detections: List[Dict]) -> np.ndarray:
        ...
        for detection in detections:
            ...
            # Draw bounding box
            cv2.rectangle(
                annotated_image, (bbox[0], bbox[1]), (bbox[2], bbox[3]), color, 2
            )
    ...
    def _add_to_history(self, detections: List[Dict]):
        ...
        self.detection_history.append(
            {"timestamp": timestamp, "detections": detections, "count": len(detections)}
        )
    ...
    def get_detection_stats(self) -> Dict:
        ...
        most_common = (
            max(class_counts.items(), key=lambda x: x[1]) if class_counts else (None, 0)
        )
