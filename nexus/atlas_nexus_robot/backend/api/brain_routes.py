...
async def chat(msg: ChatMessage):
    ...
    try:
        ...
        if msg.include_status:
            context["robot_status"] = {"status": "online", "mode": "autonomous"}
            context["systems"] = {
                "camera": True,
                "vision": True,
        ...
        response = logic_engine.reason(
            query=msg.message, context=context, conversation_history=history
        )
...
async def analyze_scene():
    ...
    try:
        ...
        return {"analysis": analysis, "detections": detections, "summary": summary}
    ...
    try:
        ...
        return {
            ...
            "ready_for_identification": (
                face_status["ready_for_identification"]
                and voice_status["ready_for_identification"]
            ),
        }
...
async def create_user_profile(request: UserProfileRequest):
    ...
    try:
        ...
        if request.birthdate:
            ...
            profile_data["birthdate"] = datetime.fromisoformat(
                request.birthdate.replace("Z", "+00:00")
            )
...
async def update_user_profile(request: UserProfileRequest):
    ...
    try:
        ...
        if "birthdate" in updates and updates["birthdate"]:
            ...
            updates["birthdate"] = datetime.fromisoformat(
...
async def personal_chat(msg: PersonalChatMessage):
    ...
    try:
        ...
        adapted_response = personality_engine.adapt_response(
            base_response, user_name=user_name, context=context
        )
...
async def update_preference(
    category: str, item: str, preference_score: float, context: Optional[Dict] = None
):
    """Update user preference"""
    try:
        profile_manager = get_profile_manager()
        result = profile_manager.update_preference(
            category, item, preference_score, context
        )
...
async def get_interaction_history(
    limit: int = 50, interaction_type: Optional[str] = None
):
...
async def get_available_ais():
    ...
    try:
        ...
        return {
            ...
            "available_count": sum(
                1 for p in providers_info.values() if p["status"] == "available"
            ),
            "timestamp": datetime.now().isoformat(),
        }
