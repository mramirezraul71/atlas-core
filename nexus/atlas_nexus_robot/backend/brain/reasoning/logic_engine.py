...
Responde siempre en español."""
    ...
    def reason(
        ...
        try:
            ...
            if context:
                ...
                messages.append(
                    SystemMessage(content=f"Contexto actual:\n{context_str}")
                )
    ...
    def _fallback_response(self, query: str, context: Optional[Dict] = None) -> str:
        ...
        if any(
            word in query_lower
            for word in ["qué ves", "que ves", "qué hay", "detectas"]
        ):
            if context and "detections" in context:
                ...
                if total > 0:
                    by_class = summary.get("by_class", {})
                    objects = ", ".join(
                        [f"{count} {obj}" for obj, count in by_class.items()]
                    )
                    return f"👁️ Detecto {total} objeto(s): {objects}"
                else:
                    return "👁️ No detecto ningún objeto en este momento."
            return (
                "👁️ Mi sistema de visión está activo pero no tengo datos de detección."
            )

        if any(
            word in query_lower
            for word in ["ayuda", "help", "qué puedes", "que puedes"]
        ):
