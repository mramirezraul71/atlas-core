...
logging.basicConfig(
    ...
    handlers=[logging.StreamHandler(), logging.FileHandler("logs/nexus.log")],
)
...
class AtlasNexus:
    ...
    def __init__(self, config: Optional[NexusConfig] = None):
        ...
        self.autonomous_engine = AutonomousEngine(
            config=self.config, router=self.router, tool_registry=self.tools_registry
        )
    ...
    async def start_api_server(self):
        ...
        logger.info(
            "Starting API Server on %s:%s", self.config.api.host, self.config.api.port
        )

        app = create_api(self)

        config = uvicorn.Config(
            app, host=self.config.api.host, port=self.config.api.port, log_level="info"
        )
