    ...
    async def create_goal(request: GoalRequest):
        try:
            plan = await nexus_engine.autonomous_engine.achieve_goal(
                goal=request.goal, context=request.context or {}
            )

            await ws_manager.broadcast(
                {"type": "goal_created", "plan_id": plan.id, "goal": plan.goal}
            )
    ...
    async def websocket_endpoint(websocket: WebSocket):
        ...
        try:
            await websocket.send_json(
                {
                    "type": "connected",
                    "message": "ATLAS NEXUS - Real-time updates active",
                }
            )
                        ...
                        await websocket.send_json(
                            {
                                ...
                                "data": {
                                    ...
                                    "active_tasks": active,
                                    "available_tools": len(
                                        nexus_engine.tools_registry.tools
                                    ),
