...
class NexusMemoryIntegration:
    ...
    def _load_memory_systems(self):
        ...
        try:
            ...
            from nexus.atlas_nexus_robot.backend.brain.memory_connector import \
                memory_connector
        ...
        if self.unified_memory:
            try:
                ...
                results = self.unified_memory.recall(
                    query=query, memory_types=memory_types, limit=10, min_relevance=0.3
                )
        ...
        if self.chat_memory and context_type == "conversation":
            try:
                ...
                if search_results["success"] and search_results["total_sessions"] > 0:
                    ...
                    logger.info(
                        f"Found {search_results['total_sessions']} relevant chat sessions"
                    )

                # Obtener contexto de sesión actual
                if self.session_id:
                    session_context = self.chat_memory.get_context_for_new_message(
                        session_id=self.session_id, new_message=query, context_limit=5
                    )
    ...
    def get_memory_summary(self) -> Dict[str, Any]:
        ...
        if self.unified_memory:
            try:
                summary["systems"][
                    "unified_memory"
                ] = self.unified_memory.get_all_stats()
            except Exception as e:
        ...
        if self.memory_connector:
            try:
                summary["systems"][
                    "memory_connector"
                ] = self.memory_connector.get_memory_statistics()
            except Exception as e:
    ...
    def enhance_prompt_with_memory(
        self, prompt: str, context_type: str = "general"
    ) -> str:
        """
        ...
        if memory_context["relevant_memories"]:
            ...
            for i, memory in enumerate(memory_context["relevant_memories"][:3]):
                enhanced_prompt += (
                    f"{i+1}. [{memory['memory_type']}] {memory['content'][:200]}...\n"
                )
...
def enhance_agent_response_with_memory(
    query: str, response: str, context_type: str = "conversation"
) -> str:
    """
