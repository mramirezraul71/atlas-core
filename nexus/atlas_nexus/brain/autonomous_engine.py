    ...
    async def execute_step(self, step: PlanStep, plan_context: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            if tool:
                ...
                logger.warning(
                    f"No tool found for action '{step.action}', using AI fallback"
                )
...
Provide the result or explain what would need to be done."""
                ...
                task_ctx = TaskContext(
                    prompt=fallback_prompt, task_type=TaskType.ANALYSIS, temperature=0.5
                )
            ...
            if step.retries < step.max_retries:
                logger.info(
                    f"Retrying step {step.id} ({step.retries}/{step.max_retries})"
                )
        ...
        while True:
            ...
            for step in ready_steps:
                ...
                if success:
                    ...
                    logger.warning(
                        f"Step {step.id} failed - continuing with remaining steps"
                    )
        ...
        if executed_plan.status == PlanStatus.FAILED:
            if self.config.autonomy.auto_recovery:
                ...
                if plan_recovery_count < self.config.autonomy.max_recovery_attempts:
                    ...
                    logger.info(
                        f"🔄 Attempting auto-recovery ({plan_recovery_count + 1}/{self.config.autonomy.max_recovery_attempts})"
                    )
...
Respond with a new goal statement that incorporates the recovery strategy."""
        ...
        task_ctx = TaskContext(
            prompt=recovery_prompt, task_type=TaskType.REASONING, temperature=0.4
        )
