...
class SupervisorMemoryIntegration:
    ...
    def store_audit_finding(self, finding: Dict[str, Any], evidence: List[Dict[str, Any]] = None):
        """
        ...
        try:
            ...
            if evidence:
                for i, ev in enumerate(evidence):
                    evidence_content = f"EVIDENCE [{i+1}]: {ev.get('type', 'unknown')}\n"
    ...
    def search_similar_issues(self, issue_description: str, component: str = None, 
        ...
        if self.unified_memory:
            try:
                ...
                if memories:
                    ...
                    logger.info(f"Found {len(memories)} similar issues in Unified Memory")
            except Exception as e:
        ...
        if self.chroma:
            try:
                ...
                logger.info(f"Found {len(audit_results)} audit findings, {len(directive_results)} directives")
    ...
    def enhance_supervisor_analysis(self, objective: str, current_snapshot: Dict[str, Any]) -> str:
        """
    ...
    def store_investigation_result(self, objective: str, investigation: Dict[str, Any], 
        ...
        try:
            ...
            investigation_content += f"Duration: {investigation.get('duration_seconds', 0)}s\n"
            investigation_content += f"Findings: {investigation.get('findings_count', 0)}\n"
            investigation_content += f"Evidence: {investigation.get('evidence_count', 0)}\n"
            
            if resolution:
                investigation_content += f"Resolution: {resolution.get('status', 'unknown')}\n"
                investigation_content += f"Actions taken: {len(resolution.get('actions', []))}\n"
            ...
            self.chroma.add_memory(
                ...
                metadata={
                    ...
                    "resolution_status": resolution.get('status') if resolution else None,
                    "timestamp": datetime.now().isoformat(),
                    "session_id": self.supervisor_session_id
                }
            )
