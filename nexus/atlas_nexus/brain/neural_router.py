...
class OllamaClient:
    ...
    def __init__(
        self, host: str = "http://localhost:11434", models: Dict[str, str] = None
    ):
        self.host = host
...
class NeuralRouter:
    ...
    def __init__(self, config):
        ...
        # Initialize clients
        self.ollama = (
            OllamaClient(host=config.ollama.host, models=config.ollama.models)
            if config.ollama.enabled
            else None
        )

        self.openai = (
            OpenAIClient(api_key=config.openai.api_key, models=config.openai.models)
        ...
        self.anthropic = (
            AnthropicClient(
                api_key=config.anthropic.api_key, models=config.anthropic.models
            )
        ...
        logger.info(
            f"Available providers: Ollama={self.ollama is not None}, "
        ...
        if self.directives_manager:
            try:
                ...
                if directives:
                    ...
                    logger.debug(
                        f"Enhanced system prompt with directives for project: {project_name}"
                    )
    ...
    async def think(self, task_ctx: TaskContext) -> ModelResponse:
        ...
        if self.config.brain.cache_responses:
            ...
            cached = self.cache.get(
                task_ctx.prompt, task_ctx.task_type.value, f"{provider}:{model}"
            )
            if cached:
        ...
        for attempt in range(self.config.brain.max_retries):
            try:
                ...
                logger.info(
                    f"Attempt {attempt + 1}: Using {provider}:{model} for {task_ctx.task_type.value}"
                )
    ...
    async def think_sync(
        ...
        """
        task_ctx = TaskContext(prompt=prompt, task_type=task_type, system_prompt=system)
