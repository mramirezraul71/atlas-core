...
def brain_think(query: str = ""):
    ...
    log_to_bitacora(
        f"Cerebro pensando: {query[:50]}...", "info", "brain", {"query": query}
    )
    try:
...
def brain_process(payload: dict):
    ...
    log_to_bitacora(
        f"Procesando: {text[:50]}...", "info", "brain", {"input_length": len(text)}
    )
    try:
        result = handle(text)
        log_to_bitacora(
...
_nervous_state = {
    ...
    "signals": {"brain": 85, "vision": 92, "audio": 78, "motor": 95, "ans": 88},
}
    ...
    try:
        if provider == "ollama":
            ...
            return {
                "ok": True,
                "provider": provider,
                "message": "Anthropic configurado (sin test directo)",
            }
            ...
            return {
                "ok": True,
                "provider": provider,
                "message": f"{provider} configurado",
            }
...
# â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
# Options Engine (paper) — snapshot para hub UI
# â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
@app.get("/api/options-engine-status")
def options_engine_status():
    """JSON para bloque OPTIONS ENGINE en /ui (paper-only)."""
    try:
...
def serve_dashboard():
    ...
    return HTMLResponse(
        content="<h1>Dashboard not found</h1><p>Path: " + str(DASHBOARD_PATH) + "</p>",
        status_code=404,
    )
...
def get_version():
    ...
    return {
        ...
        "modules": {"tutorias": TUTORIAS_ENABLED, "chat": CHAT_ENABLED},
    }
