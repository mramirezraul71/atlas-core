...
def get_camera_info(fast: bool = True) -> Dict[str, Any]:
    ...
    """
    # Prioridad: si hay cámaras online en registry, reportar la de mayor prioridad.
    try:
        online = list_cameras(only_online=True)
        if online:
            best = min(online, key=lambda x: int(x.get("priority", 99)))
            ...
            return {
                "active": True,
                "properties": {
                    "model": best.get("label") or meta.get("model", "Camera"),
                    "resolution": meta.get("resolution", [640, 480]),
                    "capabilities": meta.get("capabilities", ["video"]),
                    "url": best.get("url"),
                    "source": best.get("type"),
                },
