...
async def run_cycle(client) -> None:
    ...
    if (
        last
        and last.get("target") == cmd.get("target")
        and last.get("value") == cmd.get("value")
    ):
        logger.debug("skip duplicate command")
        return
    ok = await client.send_command(
        cmd.get("target", "NEXUS_ARM"),
        cmd.get("action", "update_state"),
        cmd.get("value", 1),
    )
    if ok:
...
if __name__ == "__main__":
    ...
    p.add_argument(
        "--seconds", type=float, default=0, help="Run N seconds then exit (0=forever)"
    )
    args = p.parse_args()
