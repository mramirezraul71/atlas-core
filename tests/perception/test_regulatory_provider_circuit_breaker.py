...
def test_regulatory_circuit_breaker_opens_after_failures(monkeypatch) -> None:
    ...
    def _boom(*args, **kwargs):  # noqa: ANN002, ANN003
        raise RuntimeError("regulatory_provider_down")

    monkeypatch.setattr("atlas_scanner.perception.regulatory.provider.requests.get", _boom)
    provider = FmpRegulatoryEventProvider(config=FmpRegulatoryProviderConfig(api_key="token"))
    now = datetime.now(timezone.utc)
    provider.fetch(symbol_or_scope="SPY", as_of=now)
    provider.fetch(symbol_or_scope="SPY", as_of=now)
    provider.fetch(symbol_or_scope="SPY", as_of=now)
