...
@dataclass
class _FailingGammaOIProvider(GammaOIProvider):
    def get_gamma_data(self, symbol: str, as_of) -> GammaData:
        _ = (symbol, as_of)
        raise RuntimeError("gamma failed")
...
def test_runner_consumes_gamma_oi_provider_and_updates_meta_and_scores(monkeypatch) -> None:
    ...
    assert selected.meta["provider_status"]["gamma_oi"]["gamma_data"] == "ok"
    assert selected.meta["provider_status"]["gamma_oi"]["oi_flow_data"] == "ok"
    assert result.meta["providers"]["gamma_oi"]["status"] == "ok"
...
def test_runner_marks_gamma_oi_empty_status_with_dummy_provider(monkeypatch) -> None:
    snapshot_symbol = SymbolSnapshot(
        symbol="SPY",
        asset_type="etf",
        base_currency="USD",
    ...
    result = run_offline_scan(
        vol_macro_provider=_StaticVolMacroProvider(),
        gamma_oi_provider=_DummyGammaOIProvider(),
    )
    assert result.meta["providers"]["gamma_oi"]["status"] == "empty"


def test_runner_handles_gamma_oi_provider_errors_without_crashing(monkeypatch) -> None:
    snapshot_symbol = SymbolSnapshot(
        symbol="SPY",
        asset_type="etf",
        base_currency="USD",
    ...
    result = run_offline_scan(
        vol_macro_provider=_StaticVolMacroProvider(),
        gamma_oi_provider=_FailingGammaOIProvider(),
    )
    assert len(result.ranked_symbols) == 1
    assert result.meta["providers"]["gamma_oi"]["status"] == "error"
    selected = result.selected_symbols[0]
