...
def test_campaign_start_picks_first_pending_lesson_and_persists(
    ...
    monkeypatch.setattr(
        "brain.learning.python_mastery_campaign.campaign_state_path",
        lambda repo_root=None: tmp_path / "state.json",
    )

    tutor = AITutor(tutor_type="disabled")
    loop = MagicMock()
    res = start_or_resume_campaign(
        tutor=tutor, loop=loop, repo_root=tmp_path, reset=True
    )
...
def test_campaign_step_advances_on_pass(tmp_path: Path, monkeypatch):
    from brain.learning.ai_tutor import AITutor
    from brain.learning.python_mastery_campaign import (
        campaign_step, start_or_resume_campaign)
    ...
    monkeypatch.setattr(
        "brain.learning.python_mastery_campaign.evaluate_python_mastery_lesson",
        fake_eval,
        raising=False,
    )
    ...
    monkeypatch.setattr(
        "brain.learning.python_mastery_evaluator.evaluate_python_mastery_lesson",
        fake_eval,
    )
    ...
    res = campaign_step(
        tutor=tutor, loop=loop, repo_root=tmp_path, lesson_id="PY001", timeout_s=10
    )
...
def test_campaign_step_retries_until_pass(tmp_path: Path, monkeypatch):
    ...
    monkeypatch.setattr(
        "brain.learning.python_mastery_campaign.time.sleep", lambda s: None
    )
                ...
                "checks": [
                    {
                        "name": "required_files",
                        "ok": False,
                        "missing": ["training/python_mastery/utils.py"],
                        "required": [],
                    },
                    {
                        "name": "pytest",
                        "ok": False,
                        "nodeids": ["tests/python_mastery/test_py001_utils.py"],
                        "result": {
                            "output": "NotImplementedError in training/python_mastery/utils.py"
                        },
                    },
                ]
            },
...
def test_campaign_run_stops_after_completion(tmp_path: Path, monkeypatch):
    ...
    monkeypatch.setattr(
        "brain.learning.python_mastery_campaign._ensure_python_curriculum",
        lambda tutor: None,
    )
