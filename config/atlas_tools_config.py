...
class AtlasToolsConfig:
    ...
    def __init__(self):
        ...
        self.OLLAMA_MODELS = ["llama2", "mistral", "codellama", "phi3"]
    ...
    def print_status(self):
        ...
        print(
            f"Composio (Conectividad): {'✅ Activo' if status['composio'] else '❌ Inactivo'}"
        )
        print(f"Ollama (Motor IA): {'✅ Activo' if status['ollama'] else '❌ Inactivo'}")
        print(
            f"Home Assistant (IoT): {'✅ Activo' if status['home_assistant'] else '❌ Inactivo'}"
        )
        print(
            f"Appsmith (Interfaz): {'✅ Activo' if status['appsmith'] else '❌ Inactivo'}"
        )
