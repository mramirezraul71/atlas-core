...
class ChatThreadManager:
    ...
    def _init_db(self):
        ...
        cursor.execute(
            "CREATE INDEX IF NOT EXISTS idx_thread_user ON chat_threads(user_id)"
        )
        cursor.execute(
            "CREATE INDEX IF NOT EXISTS idx_message_thread ON chat_messages(thread_id)"
        )
        cursor.execute(
            "CREATE INDEX IF NOT EXISTS idx_message_timestamp ON chat_messages(timestamp)"
        )
        cursor.execute(
            "CREATE INDEX IF NOT EXISTS idx_context_thread ON chat_context(thread_id)"
        )
    ...
    def create_thread(
        self, title: str, user_id: str, description: str = "", context: Dict = None
    ) -> str:
    ...
    def thread_exists(self, thread_id: str) -> bool:
        ...
        cursor = conn.cursor()
        cursor.execute(
            "SELECT 1 FROM chat_threads WHERE thread_id = ? LIMIT 1", (thread_id,)
        )
        row = cursor.fetchone()
        conn.close()
        return bool(row)
    ...
    def get_all_threads(self, user_id: str = None) -> List[Dict]:
        ...
        for row in cursor.fetchall():
            # Contar mensajes
            cursor.execute(
                "SELECT COUNT(*) FROM chat_messages WHERE thread_id = ?", (row[0],)
            )
...
if __name__ == "__main__":
    ...
    manager.add_message(
        ...
        "atlas",
        "assistant",
        "Claro, estoy aqui para ayudarte. Que necesitas?",
    )
