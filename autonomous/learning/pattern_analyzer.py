...
class PatternAnalyzer:
    def __init__(self, config: dict | None = None):
        ...
        self._min_data_points = int(
            self._config.get("pattern_analysis", {}).get("min_data_points", 100)
        )
    ...
    def analyze_usage_patterns(
        self, time_range_sec: float = 86400 * 7
    ) -> dict[str, Any]:
    ...
    def generate_insights(self) -> list[str]:
        ...
        if p.get("patterns", {}).get("by_type"):
            ...
            insights.append(
                f"Top event types: {', '.join(f'{k}({v})' for k, v in top)}"
            )
        return insights
