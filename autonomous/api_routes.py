...
def get_health_metrics_services():
    ...
    return {
        k: {
            ...
            "errors_last_5min": v.errors_last_5min,
        }
...
def post_healing_trigger():
    ...
    if not ho:
        raise HTTPException(
            status_code=503, detail="Healing orchestrator not available"
        )
    return {
        "ok": True,
        "message": "Use handle_error(error, context) for concrete errors",
    }
...
def post_evolution_scan():
    ...
    if not ev:
        raise HTTPException(
            status_code=503, detail="Evolution orchestrator not available"
        )
...
def post_learning_feedback(
    context: str = "", feedback_type: str = "satisfaction", value: float = 0.5
):
    try:
