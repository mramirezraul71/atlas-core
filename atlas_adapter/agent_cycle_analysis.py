...
class AgentCycleDetector:
    ...
    def analyze_iteration(self, iteration: int, tool_calls: List[Dict], 
                         messages: List[Dict], progress_pct: int) -> Dict[str, Any]:
        """
        ...
        if iteration > CYCLE_INDICATORS["high_iteration_count"]:
            ...
            analysis["recommendations"].append("Considerar límite de iteraciones más bajo")
        ...
        if tool_repetition:
            ...
            if tool_repetition["count"] >= CYCLE_INDICATORS["tool_repetition_threshold"]:
                analysis["risk_level"] = "high"
                analysis["recommendations"].append(f"Tool '{tool_repetition['tool']}' repetido {tool_repetition['count']} veces")
        ...
        if progress_stagnation:
            ...
            if progress_stagnation["stagnant_iterations"] >= CYCLE_INDICATORS["progress_stagnation"]:
        ...
        if self.current_cycle_start is None and analysis["risk_level"] in ["medium", "high"]:
        ...
        self.tool_calls_history.extend([
            {"iteration": iteration, "tool": tool.get("name"), "input": tool.get("input")}
    ...
    def _detect_tool_repetition(self, tool_calls: List[Dict]) -> Optional[Dict]:
        ...
        recent_tools = [
            call.get("name") for call in self.tool_calls_history[-10:]
        ]
    ...
    def _analyze_message_patterns(self, messages: List[Dict]) -> Optional[Dict]:
        ...
        if patterns_found:
            return {
                ...
                "text_sample": combined_text[:200] + "..." if len(combined_text) > 200 else combined_text
            }
    ...
    def get_cycle_summary(self) -> Dict[str, Any]:
        ...
        for analysis in self.iteration_history:
            for indicator in analysis.get("cycle_indicators", []):
                most_common_indicators[indicator] = most_common_indicators.get(indicator, 0) + 1
        
        # Ordenar por frecuencia
        sorted_indicators = sorted(
            most_common_indicators.items(), 
            key=lambda x: x[1], 
            reverse=True
        )
    ...
    def _generate_recommendations(self) -> List[str]:
        ...
        if high_risk_iterations > total_iterations * 0.3:
            recommendations.append("Considerar reducir SAFETY_MAX_ITERATIONS de 100 a 50")
        ...
        if any("progress_stagnation" in ind for ind in most_common_indicators.keys()):
            recommendations.append("Añadir lógica de escape cuando el progreso se estanca")
        ...
        for indicator, count in most_common_indicators.items():
            if indicator == "high_iteration_count" and count > 5:
                recommendations.append("Las tareas están requiriendo demasiadas iteraciones")
            elif indicator == "repetitive_analysis" and count > 3:
                recommendations.append("El LLM está analizando los mismos archivos repetidamente")
...
if __name__ == "__main__":
    ...
    for i in range(1, 55):
        ...
        if i % 10 == 0:
            tool_calls = [{"name": "execute_command", "input": {"command": "git status"}}]
        else:
        ...
        if i % 10 == 0:
            print(f"Iteración {i}: {analysis['risk_level']} - {analysis['cycle_indicators']}")
