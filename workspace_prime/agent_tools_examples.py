...
def demo_composio_connectivity():
    ...
    result = tools.composio_connect_app(
        "google_sheets",
        {"api_key": "mock-api-key", "spreadsheet_id": "trading_dashboard_sheet"},
    )
    print(f"📊 Conexión Google Sheets: {result}")

    # Ejecutar acción en Google Sheets
    result = tools.composio_execute_action(
        "google_sheets",
        "update_cell",
        {"range": "A1", "value": "Nuevo trade ejecutado"},
    )
...
def demo_appsmith_interface():
    ...
    result = tools.appsmith_create_dashboard(
        "Atlas Trading Dashboard", dashboard_config
    )
...
def demo_complete_workflow():
    ...
    tools.appsmith_create_dashboard(
        "Análisis Técnico",
