...
class WorkspaceToolsIntegration:
    ...
    def __init__(self):
        ...
        self.memory_file = self.base_dir / "workspace_prime" / "memory" / "tools_state.json"
    ...
    # MEM0 - Memoria
    def mem0_store_memory(self, content: str, tags: list = None, user_id: str = "atlas_agent") -> Dict[str, Any]:
        ...
        try:
            ...
            return {"ok": True, "memory_id": result.get("id"), "message": "Memoria almacenada (requiere OpenAI API key)"}
    ...
    def mem0_search_memories(self, query: str, user_id: str = "atlas_agent") -> Dict[str, Any]:
    ...
    def composio_connect_app(self, app_name: str, credentials: Dict[str, str]) -> Dict[str, Any]:
    ...
    def composio_execute_action(self, app_name: str, action: str, params: Dict[str, Any]) -> Dict[str, Any]:
    ...
    def home_assistant_get_devices(self) -> Dict[str, Any]:
        ...
        try:
            ...
            response = requests.get(f"{self.home_assistant_url}/api/states", headers=headers, timeout=5)
            
            if response.status_code == 200:
                devices = response.json()
                self.state["home_assistant_devices"] = [
                    {"entity_id": d["entity_id"], "state": d["state"], "attributes": d.get("attributes", {})}
    ...
    def home_assistant_control_device(self, entity_id: str, action: str) -> Dict[str, Any]:
    ...
    def n8n_get_workflows(self) -> Dict[str, Any]:
        ...
        try:
            ...
            response = requests.get(f"http://localhost:5678/api/v1/workflows", timeout=5)
            if response.status_code == 200:
                workflows = response.json().get("data", [])
                return {"ok": True, "workflows": workflows, "count": len(workflows)}
            else:
                # Si falla API, retorna estado básico
                return {"ok": True, "workflows": [], "status": "n8n activo en http://localhost:5678"}
    ...
    def n8n_create_workflow(self, name: str, nodes: list = None) -> Dict[str, Any]:
        ...
        try:
            ...
            response = requests.post(f"http://localhost:5678/api/v1/workflows", json=workflow_data, timeout=5)
            if response.status_code == 201:
                workflow = response.json().get("data", {})
                return {"ok": True, "workflow": workflow, "url": f"http://localhost:5678/workflow/{workflow.get('id')}"}
            else:
                return {"ok": False, "error": f"HTTP {response.status_code}"}
    ...
    def n8n_connect_gmail(self, credentials: Dict[str, str]) -> Dict[str, Any]:
        ...
        try:
            ...
            return {"ok": True, "connection": connection, "message": "Gmail configurado en n8n"}
    ...
    def n8n_connect_whatsapp(self, credentials: Dict[str, str]) -> Dict[str, Any]:
        ...
        try:
            ...
            return {"ok": True, "connection": connection, "message": "WhatsApp configurado en n8n"}
            
        except Exception as e:
            return {"ok": False, "error": str(e)}
    # APPSMITH - Interfaz
    def appsmith_create_dashboard(self, name: str, config: Dict[str, Any]) -> Dict[str, Any]:
...
if __name__ == "__main__":
    ...
    result = tools.mem0_store_memory("Cliente Juan prefiere productos tecnológicos", ["clientes", "juan"])
    print(f"   {result}")
