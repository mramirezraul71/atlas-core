...
class MultimodalPipeline:
    ...
    def process(self, input_data: Any, task: str = "") -> dict:
        ...
        if itype == "url":
            ...
            result["next_suggested_actions"].append(
                "Usar web_tools.WebTools().fetch(url)"
            )
            ...
            result["artifacts"].append(
                {"type": "image_b64", "path": input_data, "preview": b64[:50]}
            )
            result["actions_taken"].append("image_loaded")
            result["next_suggested_actions"].append(
                "Enviar b64 a Claude Vision via AWS Bedrock"
            )
            ...
            result["artifacts"].append(
                {"type": "file", "path": input_data, "content": content[:2000]}
            )
