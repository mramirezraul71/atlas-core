...
def workflow_trading_automation():
    ...
    dashboard_config = {
        "widgets": [
            ...
            {"type": "metric", "title": "Señal", "value": "LARGA" if "larga" in analysis['response'].lower() else "CORTA"},
            {"type": "table", "title": "Niveles Clave", "data_source": "levels"}
        ]
    }
    
    dashboard = tools.appsmith_create_dashboard("Atlas Trading Analysis", dashboard_config)
    ...
    workflow = tools.n8n_create_workflow("Trading Automation", [
        {
...
def workflow_cliente_inteligencia():
    ...
    gmail_connection = tools.composio_connect_app("gmail", {
        "email": "trading@atlas.com",
        "scope": ["send", "read"]
    })
    
    # 3. Crear dashboard de cliente
    print("\n3️⃣ Dashboard personalizado del cliente...")
    client_dashboard = tools.appsmith_create_dashboard("Cliente Intelligence", {
        "widgets": [
            {"type": "text", "content": f"Análisis: {client_analysis['response'][:200]}"},
            {"type": "metric", "title": "Preferencia", "value": "EUR/USD + GBP/JPY"},
            {"type": "chart", "title": "Rendimiento", "data_source": "client_trades"}
        ]
    })
    
    # 4. Workflow de seguimiento en n8n
    print("\n4️⃣ Workflow de seguimiento automático...")
    followup_workflow = tools.n8n_create_workflow("Cliente Followup", [
        {
...
def workflow_monitoring_iot():
    ...
    monitoring_dashboard = tools.appsmith_create_dashboard("ATLAS NEXUS Monitor", {
        "widgets": [
            {"type": "metric", "title": "Dispositivos Activos", "value": len(devices.get('devices', []))},
            {"type": "text", "content": f"Análisis: {status_text[:200]}"},
            {"type": "table", "title": "Estado Dispositivos", "data_source": "iot_devices"}
        ]
    })
    
    # 4. Workflow de alertas
    print("\n4️⃣ Configurando sistema de alertas...")
    alert_workflow = tools.n8n_create_workflow("IoT Alerts", [
        {
