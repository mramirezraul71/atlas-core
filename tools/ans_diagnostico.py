...
def main():
    ...
    print(
        f"ENV: SYSTEM_MODE={system_mode!r} GOVERNANCE_MODE={gov_mode!r} ANS_GROWTH_FORCE={ans_force!r}"
    )
    ...
    try:
        ...
        print(
            f"  ok={r.get('ok')} issues={r.get('issues_count', 0)} actions={len(r.get('actions_taken', []))} ms={r.get('ms')}"
        )
        for a in r.get("actions_taken", [])[:10]:
            print(
                f"    - {a.get('heal_id')}: ok={a.get('ok')} msg={a.get('message', '')[:60]}"
            )
    except Exception as e:
    ...
    try:
        ...
        h = run_health_verbose(
            base_url=None, active_port=int(os.getenv("ACTIVE_PORT", "8791") or 8791)
        )
        ch = h.get("checks", {})
        print(
        ...
        for k in (
            ...
            if isinstance(v, dict):
                ok = v.get(
                    "ok", v.get("writable", v.get("running", v.get("reachable", "?")))
                )
            else:
                ok = v
            print(f"  {k}: {ok}")
    ...
    try:
        ...
        open_inc = [
            i for i in get_incidents(status=None, limit=10) if i.get("status") == "open"
        ]
        ...
        for inc in open_inc[:5]:
            ...
            print(
                f"  - {inc.get('check_id')}: {inc.get('message', '')[:50]} | acciones: {acts_str}"
            )
    except Exception as e:
