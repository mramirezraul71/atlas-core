...
**Commit local (`variante/nueva`):** tras `git pull` local, `git log -1 --oneline` (última entrega amendada en sesión).  
**Commit publicado en GitHub (rama `push/quant-visual-suite-20260411`, cherry-pick sobre base limpia):** `5ce388027`  
**PR sugerido:** https://github.com/mramirezraul71/atlas-core/pull/new/push/quant-visual-suite-20260411  
**Nota:** En la rama remota se omitió la llamada a `_prewarm_quant_snapshot_caches()` en `main.py` porque esa base no incluye el subsistema de snapshot cache; el resto del comportamiento es equivalente. En `variante/nueva` esa llamada permanece si existe `_prewarm_quant_snapshot_caches` en el árbol.  
**Fecha:** 2026-04-11  
