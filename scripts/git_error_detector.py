...
class GitErrorDetector:
    ...
    def log_error(self, error_type: str, message: str, command: str):
        ...
        log_entry = {
            ...
            "message": message,
            "command": command,
            "action_required": "CALL_SPECIALIST"
    ...
    def run_git_command(self, command: str) -> dict:
        ...
        try:
            ...
            return {"success": True, "output": result.stdout, "error_type": None}
            ...
            for file_path in staged_files:
                try:
                    ...
                    if file_size > 10 * 1024 * 1024:  # 10MB
                        large_files.append(
                            {"file": file_path, "size_mb": file_size / (1024 * 1024)}
                        )
                except:
                    pass
...
"""
    ...
    def safe_push(self, remote: str = "origin", branch: str = None) -> dict:
        ...
        if not result["success"] and result["requires_specialist"]:
            ...
            # Guardar reporte
            report_file = Path(
                f"git_error_report_{datetime.now().strftime('%Y%m%d_%H%M%S')}.md"
            )
...
def main():
    ...
    parser.add_argument(
        "--check-safety", action="store_true", help="Check pre-push safety"
    )
