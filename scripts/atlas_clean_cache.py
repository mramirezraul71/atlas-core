...
def collect_pycache(actions: List[CleanAction]) -> None:
    for base in SCAN_DIRS:
        ...
        for d in base.rglob("__pycache__"):
            if d.is_dir():
                actions.append(
                    CleanAction("pycache", d, _size(d), "__pycache__ directory")
                )
...
def collect_temp_dirs(actions: List[CleanAction]) -> None:
    for d in TEMP_DIRS:
        if d.exists() and d.is_dir():
            ...
            if sz > 0:
                actions.append(
                    CleanAction("temp", d, sz, f"temporary cache dir ({_fmt(sz)})")
                )
            ...
            if all_wal or not db.exists():
                actions.append(
                    CleanAction(
                        ...
                        f,
                        _size(f),
                        "huérfano (sin .db)" if not db.exists() else "WAL file",
                    )
                )
        for f in d.glob("*.shm"):
            ...
            if all_wal or not db.exists():
                actions.append(
                    CleanAction(
                        ...
                        f,
                        _size(f),
                        "huérfano (sin .db)" if not db.exists() else "SHM file",
                    )
                )
...
def collect_old_logs(actions: List[CleanAction], max_age_days: float) -> None:
    ...
    for f in LOG_DIR.iterdir():
        ...
        name = f.name
        is_rotated = any(
            name.endswith(suf) or f".log{suf}" in name for suf in LOG_ROTATE_SUFFIXES
        )
        if is_rotated and _age_days(f) > max_age_days:
            actions.append(
                CleanAction(
                    "old_log", f, _size(f), f"log rotado {_age_days(f):.0f} días"
                )
            )
            ...
            if age > 0.01:  # > ~15 minutos
                actions.append(
                    CleanAction(
                        ...
                        lock,
                        _size(lock),
                        f"git lock huérfano {age:.1f} días",
                    )
                )
...
def main() -> int:
    ...
    ap.add_argument(
        "--execute",
        action="store_true",
        help="Ejecutar limpieza real (sin esto, solo muestra).",
    )
    ap.add_argument(
        "--no-pycache",
        action="store_true",
        help="Omitir limpieza de __pycache__ y .pyc.",
    )
    ap.add_argument(
        "--no-temp", action="store_true", help="Omitir temp_models_cache y similares."
    )
    ...
    ap.add_argument(
        ...
        type=float,
        default=3.0,
        help="Logs rotados más viejos que N días (default: 3).",
    )
    ap.add_argument(
        "--all", action="store_true", help="Incluir WAL activos (más agresivo)."
    )
    args = ap.parse_args()
    ...
    print(
        f"  Modo: {'DRY RUN (usa --execute para borrar)' if dry_run else '*** EJECUCIÓN REAL ***'}"
    )
    print(f"{'='*65}\n")
    ...
    for cat, items in by_cat.items():
        ...
        if len(items) <= 5:
            for a in items:
                rel = (
                    a.path.relative_to(REPO_ROOT)
                    if a.path.is_relative_to(REPO_ROOT)
                    else a.path
                )
    ...
    if dry_run:
        ...
        print(
            f"  {icon} Limpieza completada: {result['removed']} eliminados, {_fmt(result['freed_bytes'])} liberados."
        )
        if result["errors"]:
