...
def install_home_assistant():
    ...
    if check_docker():
        ...
        print("📝 Para iniciar: python -m homeassistant --config c:/ATLAS_PUSH/homeassistant_config")
        return True
    ...
    for tool, package in tools.items():
        try:
            if tool == "Ollama":
                ...
                print(f"{status} {tool}: {result.stdout.strip() if result.returncode == 0 else 'No disponible'}")
            elif tool == "Appsmith":
                # Verificar via Docker
                result = subprocess.run("docker ps | findstr appsmith", shell=True, capture_output=True, text=True)
                status = "✅" if result.returncode == 0 and "appsmith" in result.stdout else "❌"
                print(f"{status} {tool}: {'Corriendo en Docker' if result.returncode == 0 else 'No disponible'}")
            else:
                result = subprocess.run(f"pip show {package}", shell=True, capture_output=True, text=True)
                status = "✅" if result.returncode == 0 else "❌"
                print(f"{status} {tool}: {'Instalado' if result.returncode == 0 else 'No disponible'}")
        except Exception as e:
...
def main():
    ...
    print(f"{'✅' if ha_success else '❌'} Home Assistant: {'Instalado' if ha_success else 'Falló'}")
    print(f"{'✅' if appsmith_success else '❌'} Appsmith: {'Instalado' if appsmith_success else 'Falló (requiere Docker)'}")
