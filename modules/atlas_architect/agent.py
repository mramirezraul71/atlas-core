...
class AtlasArchitect:
    ...
    def index_project(
        self, project_root: str, name: str = "project_index"
    ) -> Dict[str, Any]:
    ...
    def run_pytest_and_analyze(
        self, nodeid: Optional[str] = None, cwd: Optional[Path] = None
    ) -> Dict[str, Any]:
        ...
        summary = (
            f"pytest FAIL. issues={len(issues)}. returncode={res.get('returncode')}"
        )
    ...
    def propose_code_fix(
        ...
        out = self.models.run(
            prompt, intent_hint="code", prefer_free=prefer_free, timeout_s=60
        )
        self.ans.log_suggestion(
            f"Propuesta generada (modelo={out.model_routing.get('model_key')}). Motivo: {out.model_routing.get('reason')}"
        )
    ...
    def execute_order(
        ...
        *,
        ...
        mode_norm = (
            "growth" if mode_norm in ("growth", "aggressive", "auto") else "governed"
        )
        ...
        if is_inventory:
            if mode_norm == "governed":
                plan = self.scaffold.plan_inventory_app(
                    org_name=org_name, app_name="inventario", mode="governed"
                )
                try:
                    from modules.humanoid.approvals import \
                        create as approvals_create
                ...
                self.ans.log_suggestion(
                    "[ARCHITECT] Diseñando arquitectura para nueva App... (GOVERNED)"
                )
                return {
                    "ok": True,
                    "mode": "governed",
            ...
            self.ans.log_suggestion(
                "[ARCHITECT] Diseñando arquitectura para nueva App... (GROWTH)"
            )
            sc = self.scaffold.scaffold_inventory_app(
                org_name=org_name, app_name="inventario"
            )
            ...
            deps_ok = False
            if vinfo.ok:
                deps_ok = self.venv.install_requirements(
                    vinfo.python_exe, backend_dir / "requirements.txt", cwd=backend_dir
                )

            test_cmd = (
                f'"{vinfo.python_exe}" -m pytest -q'
                if vinfo.ok
        ...
        self.ans.log_suggestion(
            "Tipo de app no reconocido por heurística; generando propuesta."
        )
        prop = self.propose_code_fix(
            f"Diseñar app desde orden: {text}",
            context={"arch": arch, "repo_index": repo_idx},
            prefer_free=prefer_free,
        )
    ...
    def apply_approved_plan(self, approval_id: str) -> Dict[str, Any]:
        ...
        plan = ChangePlan(
            goal=payload.get("goal") or "approved_plan", mode="growth", changes=[]
        )
        ...
        self.ans.log_suggestion(
            f"Plan aprobado aplicado: approval_id={approval_id} cambios={len(applied)}"
        )
    ...
    def capture_and_ocr_screen(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if err or not img:
                ...
                return {
                    "ok": False,
                    "error": err or "capture_failed",
                    "text": "",
                    "evidence_path": None,
                }
            evidence_path = save_capture_to_file(
                img,
                dir_path=str(self.repo_root / "snapshots" / "architect"),
                prefix="architect_screen",
            )
    ...
    def agentic_execute(
        ...
        *,
        ...
        tools_spec = [
            ...
            {
                "name": "atomic_patch_apply",
                "args": {"path": "string", "ops": "list", "justification": "string"},
            },
            {"name": "create_directory", "args": {"path": "string"}},
            {
                "name": "list_files",
                "args": {"path": "string", "pattern": "string", "limit": "int"},
            },
        ...
        for it in range(max(1, min(8, int(max_iters or 3)))):
            ...
            model = self.models.run(
                prompt, intent_hint="architect", prefer_free=prefer_free, timeout_s=60
            )
            ...
            self.ans.log_suggestion(
                f"ATLAS_ARCHITECT iter={it+1}: plan recibido. done={bool(parsed.get('done'))}"
            )
            ...
            for call in calls[:10]:
                ...
                try:
                    if name == "read_file":
                        ...
                        out["result"] = self.fs.atomic_patch_preview(
                            args.get("path") or "", ops
                        )
                        out["ok"] = True
                ...
                self.ans.log_suggestion(
                    f"ATLAS_ARCHITECT tool={name} ok={out.get('ok')}"
                )
