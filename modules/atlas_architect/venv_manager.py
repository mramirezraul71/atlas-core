...
class VenvManager:
    ...
    def __init__(
        self, term: TerminalTools, ans: Optional[ANSBitacoraLogger] = None
    ) -> None:
        self.term = term
        self.ans = ans
    ...
    def ensure_venv(self, project_dir: Path, venv_name: str = ".venv") -> VenvInfo:
        ...
        if self.ans:
            self.ans.log_suggestion(
                f"[ARCHITECT] Creando VENV para proyecto: {venv_dir}"
            )
        r = self.term.run_command(
            f'python -m venv "{venv_dir}"', timeout_s=120, cwd=project_dir
        )
        if not r.get("ok"):
            return VenvInfo(
                ok=False,
                ...
                python_exe=py,
                error=r.get("error") or "venv_create_failed",
            )
        return VenvInfo(ok=True, venv_dir=str(venv_dir), python_exe=py, error=None)

    def install_requirements(
        self, python_exe: str, requirements_path: Path, *, cwd: Path
    ) -> bool:
        ...
        self.term.run_command(
            f'"{python_exe}" -m pip install -U pip', timeout_s=180, cwd=cwd
        )
        r = self.term.run_command(
