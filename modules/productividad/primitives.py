...
def _db_path() -> Path:
    return Path(
        (
            os.getenv("PRODUCTIVITY_DB_PATH")
            or r"C:\ATLAS_PUSH\logs\productivity.sqlite"
        ).strip()
    )
...
def schedule_event(title: str, time_text: str, desc: str = "") -> Dict[str, Any]:
    ...
    conn = _ensure()
    cur = conn.execute(
        "INSERT INTO events (ts, title, when_text, desc) VALUES (?,?,?,?)",
        (time.time(), title[:200], time_text[:200], desc[:1000]),
    )
    conn.commit()
    ...
    try:
        ...
        emit(
            "productividad",
            f"Recordatorio creado: {title}",
            level="info",
            data={"event_id": eid, "when": time_text},
        )
    except Exception:
        pass
    return {
...
def check_rauli_inventory(
    camera_id: str = "", *, use_llm_vision: bool = False
) -> Dict[str, Any]:
    """
    ...
    try:
        ...
        ws = capture_world_state(
            eye=eye, include_ocr_items=False, use_llm_vision=bool(use_llm_vision)
        )
