...
def _vault_candidate_paths() -> list[Path]:
    """Mismas fuentes que ``atlas_http_api`` (Bóveda) + rutas habituales."""
    out: list[Path] = []
...
def _load_env() -> None:
    """Carga .env global/local y luego la Bóveda (override) para KALSHI_* y claves."""
    try:
...
def get_settings() -> RadarSettings:
    ...
    """
    _load_env()
    # Paper: capital virtual por defecto si no vino de env / bóveda
...
def reload_settings() -> RadarSettings:
    """Invalida caché y vuelve a leer Bóveda + .env (p. ej. al montar en :8791)."""
