...
def _mode_allows() -> bool:
    try:
        from modules.humanoid.mode.config import (
            is_auto_plugin_expansion_enabled, is_observe_only)
...
def create_plugin(
    name: str, task_description: str = "", base_path: Optional[str] = None
) -> Dict[str, Any]:
    ...
    if not _mode_allows():
        return {
            "ok": False,
            "path": None,
            "error": "plugin_expansion disabled or observe_only",
        }
    base = (
        base_path
    ...
    try:
        ...
        record_evolution(
            "create_plugin",
            goal=name,
            spec_json={"task": task_description},
            outcome="created",
            ok=True,
        )
    except ImportError:
...
def register_plugin(name: str, dispatcher_path: str = "") -> Dict[str, Any]:
    ...
    try:
        ...
        record_evolution(
            "register_plugin",
            goal=name,
            spec_json={"dispatcher": dispatcher_path},
            outcome="registered",
            ok=True,
        )
    except ImportError:
