...
class BaseTool(ABC):
    ...
    async def validate_parameters(self, parameters: Dict[str, Any]) -> bool:
        ...
        required = [
            k for k, v in self.metadata.parameters.items() if v.get("required", False)
        ]
            ...
            ToolMetadata(
                ...
                parameters={
                    "packages": {
                        "type": "string",
                        "required": True,
                        "description": "Package(s), e.g. pytesseract pywinauto",
                    },
    ...
    async def execute(self, parameters: Dict[str, Any], context: Dict[str, Any]) -> Any:
        ...
        try:
            ...
            if action_id:
                try:
                    action_log.finish(
                        action_id, status="ok" if ok else "error", output=out[:3000]
                    )
                except Exception:
                    pass
            return {
