...
from modules.humanoid.cluster import (registry, remote_client, remote_server,
                                      router, trace)
...
def _run_local_hands(
    command: str, timeout_sec: int = 30, **kwargs: Any
) -> Dict[str, Any]:
    return remote_server.execute_remote_hands(
        {"command": command, "timeout_sec": timeout_sec, **kwargs}
    )


def _run_local_web(action: str, timeout_sec: int = 30, **kwargs: Any) -> Dict[str, Any]:
    return remote_server.execute_remote_web(
        {"action": action, "url": action, "timeout_sec": timeout_sec, **kwargs}
    )
...
def run_hands(
    ...
    **kwargs: Any,
    ...
    decision = router.route_decision(
        "hands", prefer_remote=prefer_remote, require_capability="hands"
    )
    if (
        decision.get("route") == "remote"
        and decision.get("base_url")
        and is_action_allowed_for_node(decision.get("node_id", ""), "hands")
    ):
        resp = remote_client.post_json(
            decision["base_url"],
            "/remote/hands",
            {"command": command, "timeout_sec": timeout_sec, **kwargs},
        ...
        if resp.get("ok") and body.get("ok"):
            return _std_result(
                ...
                body.get("ms", ms),
                body.get("error"),
                body.get("correlation_id") or cid,
            )
        return _std_result(
            False,
            body.get("data"),
            ms,
            body.get("error") or resp.get("error", "remote failed"),
            cid,
        )
    r = _run_local_hands(command, timeout_sec, **kwargs)
    return _std_result(
...
def run_web(
    ...
    **kwargs: Any,
    ...
    decision = router.route_decision(
        "web", prefer_remote=prefer_remote, require_capability="web"
    )
    if (
        decision.get("route") == "remote"
        and decision.get("base_url")
        and is_action_allowed_for_node(decision.get("node_id", ""), "web")
    ):
        resp = remote_client.post_json(
            decision["base_url"],
            "/remote/web",
            {"action": action, "timeout_sec": timeout_sec, **kwargs},
            timeout_sec=REMOTE_TIMEOUT,
        )
        body = resp.get("data") or {}
        ms = int((time.perf_counter() - t0) * 1000)
...
def run_vision(
    ...
    **kwargs: Any,
    ...
    decision = router.route_decision(
        "vision", prefer_remote=prefer_remote, require_capability="vision"
    )
    if (
        decision.get("route") == "remote"
        and decision.get("base_url")
        and is_action_allowed_for_node(decision.get("node_id", ""), "vision")
    ):
        resp = remote_client.post_json(
            decision["base_url"],
            "/remote/vision",
...
def run_voice(
    ...
    **kwargs: Any,
    ...
    decision = router.route_decision(
        "voice", prefer_remote=prefer_remote, require_capability="voice"
    )
    if (
        decision.get("route") == "remote"
        and decision.get("base_url")
        and is_action_allowed_for_node(decision.get("node_id", ""), "voice")
    ):
        resp = remote_client.post_json(
            decision["base_url"],
            "/remote/voice",
