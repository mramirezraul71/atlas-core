...
def _audit(
    ...
    try:
        ...
        get_audit_logger().log_event(
            "deploy", "bluegreen", "bluegreen", action, ok, ms, error, payload, None
        )
    except Exception:
        pass


def run_bluegreen_flow(
    actor: Any = None, dry_run: bool = False, ref: Optional[str] = None
) -> Dict[str, Any]:
    """
    ...
    auto_switch = state.get(
        "auto_switch_on_health", _env_bool("AUTO_SWITCH_ON_HEALTH", True)
    )
    ...
    if not wait_until_healthy(base_staging, timeout_sec=TIMEOUT_START_SEC):
        ...
        state["last_deploy"] = {
            ...
            "ref": ref,
            "result": "rollback",
            "error": "staging did not become healthy",
        }
        persist_state(state)
        _audit(
            "rollback",
        ...
        return {
            "ok": False,
            "data": {"steps": steps, "rollback": "staging unhealthy"},
            "ms": int((time.perf_counter() - t0) * 1000),
            "error": "staging unhealthy",
        }
    ...
    steps.append(
        {"step": "smoke_result", "ok": smoke_ok, "error": smoke_result.get("error")}
    )
    _audit(
        "smoke_result",
        smoke_ok,
        {"ok": smoke_ok, "error": smoke_result.get("error")},
    ...
    if not smoke_ok:
        ...
        return {
            "ok": False,
            "data": {"steps": steps, "rollback": "smoke failed"},
            "ms": int((time.perf_counter() - t0) * 1000),
            "error": "smoke failed",
        }
    ...
    _audit(
        "health_scores",
        health_ok,
        {"steps": steps, "last_health": last_health},
        None if health_ok else "score < min",
        0,
    )
    if not health_ok:
        stop_by_pid(staging_pid)
        ...
        state["last_deploy"] = {
            ...
            "ref": ref,
            "result": "rollback",
            "error": f"health score < {min_score}",
        }
        persist_state(state)
        _audit(
            "rollback",
        ...
        return {
            "ok": False,
            "data": {"steps": steps, "rollback": "health failed"},
            "ms": int((time.perf_counter() - t0) * 1000),
    ...
    if dry_run:
        ...
        return {
            "ok": True,
            "data": {"steps": steps, "switched": False, "dry_run": True},
            "ms": ms,
            "error": None,
        }
    ...
    if not auto_switch:
        ...
        return {
            ...
            "data": {
                "steps": steps,
                "switched": False,
                "message": "AUTO_SWITCH_ON_HEALTH=false",
            },
            "ms": int((time.perf_counter() - t0) * 1000),
            "error": None,
        }
    ...
    state["last_health"] = {
        "score": last_health.get("score"),
        "checks": last_health.get("checks"),
    }
    ...
    try:
        from modules.humanoid.update.git_manager import (checkout_branch,
                                                         merge_staging)
    ...
    if os.getenv("SERVICE_ENABLED", "").strip().lower() in ("1", "true", "yes"):
        try:
            ...
            return {
                "ok": False,
                "data": {"steps": steps, "switched": False, "error": str(e)},
                "ms": int((time.perf_counter() - t0) * 1000),
                "error": str(e),
            }
    _audit(
        "promote_success",
        True,
        {"steps": steps, "active_port": active_port},
        None,
        int((time.perf_counter() - t0) * 1000),
    )
    try:
    ...
    return {
        "ok": True,
        "data": {"steps": steps, "switched": True},
        "ms": int((time.perf_counter() - t0) * 1000),
        "error": None,
    }


def _run_ga_post_update(
    active_port: int, last_health: Dict[str, Any], min_score: int
) -> None:
    ...
    if not _env_bool("GA_POST_UPDATE_AUTOCHECK", True):
        return
    try:
        ga_enabled = os.getenv("GOVERNED_AUTONOMY_ENABLED", "true").strip().lower() in (
            "1",
            "true",
            "yes",
        )
        ...
        if score < ga_min:
            try:
                from modules.humanoid.approvals.store import \
                    create as approval_create
                ...
                approval_create(
                    ...
                    payload={
                        "score": score,
                        "min": ga_min,
                        "reason": "post-deploy health below threshold",
                    },
                    risk="critical",
                )
                _audit(
