...
class SensorReading:
    ...
    timestamp_ns: int = field(default_factory=lambda: time.time_ns())
    data: Union[
        VisionFrame, IMUReading, ForceTorqueReading, EncoderReading, dict
...
class WorldState:
    ...
    def nearest_human(self) -> Optional[HumanState]:
        ...
        def dist(h):
            return (
                (h.pose.x - self.robot_pose.x) ** 2
                + (h.pose.y - self.robot_pose.y) ** 2
            ) ** 0.5
...
class VitalsStatus:
    ...
    def is_critical(self) -> bool:
        return (
            ...
            or any(t > 70 for t in self.motor_temps.values())
        )
