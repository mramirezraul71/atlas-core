...
def bootstrap_comms(
    ...
    with _lock:
        ...
        if "webhook" in services:
            ...
            status.enabled = os.getenv("MAKEPLAY_ENABLED", "true").strip().lower() in (
                "1",
                "true",
                "yes",
            )
        ...
        if "audio" in services:
            ...
            status.enabled = os.getenv("OPS_AUDIO_ENABLED", "true").strip().lower() in (
                "1",
                "true",
                "yes",
            )
        ...
        if "makeplay" in services:
            ...
            webhook_url = (
                os.getenv("MAKEPLAY_WEBHOOK_URL")
                or os.getenv("EXTERNAL_WEBHOOK_URL")
                or ""
            ).strip()
            status.enabled = bool(webhook_url) and os.getenv(
    ...
    if result["ok"]:
        logger.info(
            f"Bootstrap completado en {duration_ms:.0f}ms - {len(_services)} servicios"
        )
    else:
    ...
    with _lock:
        try:
            ...
            if service_name in _services:
                ...
                if ok:
                    _services[service_name].started_at = datetime.now(
                        timezone.utc
                    ).isoformat()
