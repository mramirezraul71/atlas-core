...
def _enabled() -> bool:
    ...
    scan = os.getenv("MAKEPLAY_SCAN_ENABLED", "true").strip().lower() in (
        "1",
        "true",
        "yes",
        "y",
    ...
    forced = os.getenv("MAKEPLAY_ENABLED", "").strip().lower() in (
        "1",
        "true",
        "yes",
        "y",
...
def ensure_makeplay_jobs() -> None:
    ...
    try:
        ...
        scanner_job = next(
            (j for j in jobs if j.get("name") == "makeplay_scanner"), None
        )
        if scanner_job:
            conn = db._ensure()
            conn.execute(
