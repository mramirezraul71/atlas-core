...
class CommsHub:
    ...
    def _send_telegram(self, msg: CommsMessage) -> Dict[str, Any]:
        ...
        try:
            ...
            if msg.evidence_path and str(msg.evidence_path).lower().endswith(
                ...
                result = bridge.send_photo(
                    chat_id, str(msg.evidence_path), caption=text[:900]
                )
            else:
    ...
    def _send_ops(self, msg: CommsMessage) -> Dict[str, Any]:
        ...
        try:
            ...
            _RECENT.append(ev)
            _append_log(
                f"{ev['ts']} [{ev['level']}] {ev['subsystem']}: {ev['message']}"
            )
            return {"ok": True}
        except Exception as e:
            return {"ok": False, "error": str(e)}
    ...
    def _get_sender(
        self, channel: str
    ) -> Optional[Callable[[CommsMessage], Dict[str, Any]]]:
    ...
    def send(
        ...
        for channel in channels:
            ...
            if success and result and result.get("ok"):
                ...
                error = (
                    str(exc) if exc else (result.get("error") if result else "unknown")
                )
        ...
        with self._lock:
            ...
            for name, health in self._channel_health.items():
                ...
                channels[name] = {
                    ...
                    "last_error": health.last_error[:100]
            ...
            any_critical_failed = any(
                h.state == ChannelState.FAILED and h.name in ("telegram", "ops")
                for h in self._channel_health.values()
            )
            ...
            return {
                ...
                "status": "healthy"
                if all_healthy
                else ("degraded" if not any_critical_failed else "unhealthy"),
                "channels": channels,
