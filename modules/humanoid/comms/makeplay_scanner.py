...
def run_scan() -> Dict[str, Any]:
    ...
    try:
        ...
        emit(
            "scanner_end",
            message=f"health={snapshot.get('health_score')} incidents={snapshot.get('incidents_open')} webhook={snapshot.get('webhook_pushed')}",
            details={
                "health_score": snapshot.get("health_score"),
                "incidents_open": snapshot.get("incidents_open"),
            },
        )
    except Exception:
        pass
        ...
        out["last_actions"] = [
            {
                ...
                "heal_id": e.get("heal_id"),
                "message": (e.get("message") or "")[:60],
            }
            for e in recent
            if e.get("phase") in ("heal_end", "incident", "cycle_end")
        ]
    except Exception:
        pass
    try:
