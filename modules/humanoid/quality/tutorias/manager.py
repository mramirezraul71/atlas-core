...
from .models import (Especialista, EstadoRecomendacion, Evaluacion, Informe,
                     NivelEvaluacion, PrioridadRecomendacion, Recomendacion,
                     SeguimientoMejora, TipoVisita, Visita)
...
class TutoriasManager:
    ...
    def obtener_especialista(self, especialista_id: str) -> Optional[Especialista]:
        ...
        cursor.execute(
            "SELECT data_json FROM especialistas WHERE id = ?", (especialista_id,)
        )
        row = cursor.fetchone()
        conn.close()
    ...
    def listar_informes(
        self, limite: int = 50, solo_firmados: bool = False
    ) -> List[Informe]:
        """Lista informes"""
        conn = sqlite3.connect(self.db_path)
        cursor = conn.cursor()
    ...
    def actualizar_estado_recomendacion(
        self, recomendacion_id: str, estado: EstadoRecomendacion, nota: str = None
    ):
        ...
        cursor.execute(
            "SELECT data_json FROM recomendaciones WHERE id = ?", (recomendacion_id,)
        )
        row = cursor.fetchone()
        ...
        if row:
            ...
            if nota:
                ...
                data["notas_seguimiento"].append(
                    f"[{datetime.now().isoformat()}] {nota}"
                )
    ...
    def listar_recomendaciones(
        self, estado: EstadoRecomendacion = None, modulo: str = None, limite: int = 50
    ) -> List[Recomendacion]:
    ...
    def crear_seguimiento(
        self, recomendacion: Recomendacion, responsable: str = "ATLAS"
    ) -> SeguimientoMejora:
    ...
    def actualizar_seguimiento(
        self, seguimiento_id: str, hito_idx: int = None, completado: bool = True
    ):
        ...
        cursor.execute(
            "SELECT data_json FROM seguimientos WHERE id = ?", (seguimiento_id,)
        )
        row = cursor.fetchone()
    ...
    def listar_seguimientos(
        self, solo_activos: bool = True, limite: int = 50
        ...
        for row in rows:
            ...
            if data.get("ultima_actualizacion"):
                seg.ultima_actualizacion = datetime.fromisoformat(
    ...
    def obtener_estadisticas(self) -> Dict[str, Any]:
        ...
        # Puntuación promedio
        cursor.execute(
            "SELECT AVG(puntuacion_global) FROM informes WHERE puntuacion_global > 0"
        )
