...
def list_pots(
    ...
    for pot in pots:
        ...
        if category:
            pot_cat = (
                pot.category.value
                if isinstance(pot.category, POTCategory)
                else pot.category
            )
            if pot_cat != category:
                continue
        ...
        if severity:
            pot_sev = (
                pot.severity.value if hasattr(pot.severity, "value") else pot.severity
            )
            if pot_sev != severity:
                continue
...
def get_pot_by_incident(
    ...
    _log.info(
        "Selected POT %s (score=%d) for check_id=%s", best_pot.id, best_score, check_id
    )
    return best_pot
