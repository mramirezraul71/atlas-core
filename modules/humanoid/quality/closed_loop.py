...
class ClosedLoopEngine:
    ...
    def register_verifier(
        self, pot_id: str, verifier: Callable[[LoopContext], bool]
    ) -> None:
        ...
        try:
            ...
            if not success:
                ...
                if ctx.attempts < ctx.max_attempts:
                    _log.warning(
                        "[LOOP:%s] POT failed, retrying (%d/%d)",
                        ctx.id,
                        ctx.attempts,
                        ctx.max_attempts,
                    )
            ...
            if not verified:
                ...
                if ctx.attempts < ctx.max_attempts:
                    _log.warning(
                        "[LOOP:%s] Verification failed, retrying (%d/%d)",
                        ctx.id,
                        ctx.attempts,
                        ctx.max_attempts,
                    )
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        with self._lock:
            ...
            success = sum(
                1 for c in self._completed_loops if c.outcome == LoopOutcome.SUCCESS
            )
            failed = sum(
                1 for c in self._completed_loops if c.outcome == LoopOutcome.FAILED
            )
