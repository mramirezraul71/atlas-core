...
class POTStep:
    ...
    expected_output: Optional[
        str
    ] = None  # Para COMMAND/SHELL (substring esperado en stdout)
    ...
    on_failure: Optional[
        str
    ] = None  # Compatibilidad: hint (no controla flujo en executor)
    ...
    manual_instructions: str = (
        ""  # Compatibilidad: instrucciones extensas (StepType.MANUAL)
    )
...
class POT:
    ...
    def to_dict(self) -> Dict[str, Any]:
        return {
            ...
            if isinstance(self.category, POTCategory)
            else self.category,
            "severity": self.severity.value
            if isinstance(self.severity, POTSeverity)
            else self.severity,
