...
class RewardEngine:
    ...
    def emit_reward(
        self, value: float, source: str, reason: str, context: Dict[str, Any] = None
    ) -> RewardSignal:
        """
        ...
        # Log significant rewards (high positive or negative)
        task_id = (
            context.get("task_id", context.get("goal_id", "unknown"))
            if context
            else "unknown"
        )
        if abs(value) > 0.3:
    ...
    def emit_goal_reward(
        self, success: bool, progress: float = 1.0, efficiency: float = 1.0
    ) -> RewardSignal:
        """
    ...
    def emit_user_feedback(
        self, positive: bool, intensity: float = 1.0
    ) -> RewardSignal:
        """
    ...
    def emit_safety_penalty(self, severity: float, violation_type: str) -> RewardSignal:
        """
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "average_reward": self._total_rewards / self._reward_count
