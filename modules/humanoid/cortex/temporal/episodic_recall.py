...
class EpisodicRecall:
    ...
    def _score_episodes(
        ...
        for ep in episodes:
            ...
            outcome = (
                outcome_raw.value if hasattr(outcome_raw, "value") else str(outcome_raw)
            )
        ...
        if ep_embedding and query.text and self.embedding_model:
            ...
            try:
                ...
                similarity = np.dot(ep_embedding, query_embedding) / (
                    np.linalg.norm(ep_embedding) * np.linalg.norm(query_embedding)
                    + 1e-8
                )
