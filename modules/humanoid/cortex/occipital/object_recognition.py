...
class EmbeddingExtractor:
    ...
    def extract(
        self, image: Any, bbox: Optional[Tuple[int, int, int, int]] = None
    ) -> List[float]:
        """
...
class ObjectRecognition:
    ...
    def _recognize_detection(
        ...
        if best_match:
            ...
            _bitacora(
                f"Object recognized: {identity.name} ({identity.category}) - confidence: {best_similarity:.2f}"
            )
            return identity
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "avg_processing_time_ms": self._total_time_ms
            / max(1, self._recognition_count),
