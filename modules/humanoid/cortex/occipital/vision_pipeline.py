...
class VisionOutput:
    ...
    def get_detection_by_class(self, class_name: str) -> List[Detection]:
        ...
        return [
            d for d in self.detections if d.class_name.lower() == class_name.lower()
        ]
...
class VisionPipeline:
    ...
    def _load_depth_estimator(self) -> None:
        ...
        try:
            ...
            self._depth_estimator = DepthEstimation(
                model_name="midas", model_size="MiDaS_small"
            )
    ...
    def process_frame(
        ...
        _bitacora(
            f"Vision: {len(output.detections)} detections, {len(output.humans)} humans"
        )
        return output
