...
class SpatialMap:
    ...
    def add_landmark(self, landmark: Landmark) -> None:
        ...
        if is_new:
            _bitacora(
                f"Landmark added: {landmark.name} ({landmark.landmark_type}) at ({landmark.position[0]:.2f}, {landmark.position[1]:.2f}, {landmark.position[2]:.2f})"
            )
    ...
    def get_path_clear(
        self, target_x: float, target_y: float, check_interval: float = 0.1
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "current_region": self.get_current_region().name
            if self.get_current_region()
            else None,
