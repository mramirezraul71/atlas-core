...
class JointState:
    ...
    def is_at_limit(self, margin: float = 0.05) -> bool:
        ...
        return (
            self.position <= self.min_position + margin
            or self.position >= self.max_position - margin
        )
...
class BodySchema:
    ...
    def __init__(self, config: Dict[str, Any] = None):
        ...
        joint_config = (
            config.get("joints", self.DEFAULT_JOINTS) if config else self.DEFAULT_JOINTS
        )
        ...
        part_config = (
            config.get("parts", self.DEFAULT_PARTS) if config else self.DEFAULT_PARTS
        )
    ...
    def get_joint_positions(self, part_id: str = None) -> Dict[str, float]:
        ...
        return {
            jid: self.joints[jid].position for jid in joint_ids if jid in self.joints
        }

    def set_joint_positions(
        self, positions: Dict[str, float], validate: bool = True
    ) -> Dict[str, str]:
        """
    ...
    def get_joints_at_limits(self) -> List[str]:
        ...
        joints_at_limits = [
            jid for jid, joint in self.joints.items() if joint.is_at_limit()
        ]
        if joints_at_limits:
    ...
    def get_overheating_joints(self, threshold: float = 60.0) -> List[str]:
        ...
        overheating = [
            jid for jid, joint in self.joints.items() if joint.temperature > threshold
        ]
        if overheating:
            temps = {jid: self.joints[jid].temperature for jid in overheating}
            _bitacora(
                f"Overheating detected: {', '.join(f'{jid}({temps[jid]:.1f}°C)' for jid in overheating)}",
                ok=False,
            )
        return overheating
