...
class Context:
    ...
    def is_safety_focus(self) -> bool:
        return (
            self.mode == "safety_focus"
            or self.internal_state.get("safety_priority", 0) > 0.7
        )
...
class UtilityFunction:
    ...
    def evaluate(self, option: ActionOption, context: Context) -> float:
        ...
        scores["efficiency"] = 1.0 - min(
            option.estimated_duration_s / max_duration, 1.0
        )
        ...
        utility = sum(
            self.weights[factor] * scores.get(factor, 0) for factor in self.weights
        )
    ...
    def update_preference(
        self, action_type: str, description: str, reward: float
    ) -> None:
...
class RiskEvaluator:
    ...
    def evaluate(self, option: ActionOption, context: Context) -> float:
        ...
        # Combinar
        total_risk = (
            base_risk + estimated_risk
        ) / 2 * context_multiplier + history_factor
...
class DecisionMaker:
    ...
    def decide(self, options: List[ActionOption], context: Context) -> Decision:
        ...
        if best["risk"] > self.max_acceptable_risk and not context.urgency > 0.9:
            logger.warning(
                f"High risk action selected: {best['option'].description}, risk={best['risk']:.2f}"
            )
            _bitacora(
                f"Decisión de alto riesgo: {best['option'].description} risk={best['risk']:.2f}",
                ok=False,
            )
    ...
    def feedback(
        self, action: ActionOption, success: bool, reward: float = None
    ) -> None:
        """
