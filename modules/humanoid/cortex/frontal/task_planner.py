...
class PDDLPlanner:
    ...
    ACTIONS = {
        ...
        "grasp": {
            "preconditions": [
                "robot_at(?location)",
                "object_at(?obj, ?location)",
                "hand_empty",
            ],
            "effects": [
                "holding(?obj)",
                "not object_at(?obj, ?location)",
                "not hand_empty",
            ],
            "duration": 5.0,
        },
...
class TaskPlanner:
    ...
    async def plan(
        self, goal: str, world_state: Any = None, context: Dict = None
    ) -> TaskPlan:
        """
        ...
        try:
            ...
            if symbolic_plan and symbolic_plan.is_valid():
                ...
                _bitacora(
                    f"Plan simbólico generado: {goal} ({len(symbolic_plan.steps)} pasos)"
                )
        ...
        if self.llm_client:
            try:
                ...
                if llm_plan and llm_plan.is_valid():
                    ...
                    _bitacora(
                        f"Plan LLM generado: {goal} ({len(llm_plan.steps)} pasos)"
                    )
                    return llm_plan
            except Exception as e:
    ...
    async def _plan_with_llm(
        self, goal: str, world_state: Any, context: Dict
    ) -> TaskPlan:
