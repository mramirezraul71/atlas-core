...
def run() -> dict:
    try:
        ...
        return {
            "ok": True,
            "check_id": "cluster_health",
            "message": f"nodes={len(nodes)} online={online}",
            "details": {"nodes": len(nodes), "online": online},
            "severity": "low",
            "suggested_heals": ["mark_node_offline"] if online < len(nodes) else [],
        }
