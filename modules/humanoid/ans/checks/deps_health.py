...
def run() -> dict:
    try:
        ...
        critical_missing = [
            m for m in missing if not any(v in m.lower() for v in voice_deps)
        ]

        ok = len(critical_missing) == 0 or all(
            m in ("ollama running",) for m in critical_missing
        )
        suggested_heals = []
        message = f"missing={critical_missing}" if critical_missing else "ok"
        details = {
            "missing_deps": critical_missing,
        ...
        if "tesseract" in critical_missing and not _tesseract_binary_exists():
            ...
            ] = "Intervención Humana Requerida: Instalar Tesseract OCR en el OS si winget falla."
            ...
            pip_installable = [
                m for m in critical_missing if m not in ("ollama running",)
            ]
            if pip_installable:
        ...
        if getattr(e, "errno", None) in (22, 9):
            return {
                "ok": True,
                "check_id": "deps_health",
                "message": "ok (io_error_suppressed)",
                "details": {"suppressed_error": str(e)},
                "severity": "low",
                "suggested_heals": [],
            }
        return {
