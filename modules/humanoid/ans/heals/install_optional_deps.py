...
_PKG_COMMENT = {
    "pytesseract": {
        "es": "OCR: lectura de texto en imágenes",
        "en": "OCR: read text from images",
    },
    "pywinauto": {
        "es": "Automatización de ventanas Windows",
        "en": "Windows UI automation",
    },
    "pillow": {"es": "Procesamiento de imágenes", "en": "Image processing"},
    "mss": {"es": "Captura de pantalla rápida", "en": "Fast screen capture"},
    "pyautogui": {
        "es": "Control de teclado y ratón",
        "en": "Keyboard and mouse control",
    },
    "playwright": {
        "es": "Automatización de navegadores web",
        "en": "Web browser automation",
    },
    "faster-whisper": {
        "es": "Transcripción de voz (STT)",
...
def run(**kwargs) -> dict:
    if os.getenv("ANS_INSTALL_OPTIONAL_DEPS", "").strip().lower() not in (
        "1",
        "true",
        "yes",
    ):
        return heal_result(
            False, "install_optional_deps", "ANS_INSTALL_OPTIONAL_DEPS not enabled", {}
        )
    try:
    ...
    if "tesseract" in missing:
        return heal_result(
            ...
            "Falta el binario Tesseract en el OS; no usar pip. Ejecutar heal install_tesseract (winget).",
            {"skip_pip": True},
        )
    to_install = [
        p
        for p in missing
        if p in _WHITELIST and p not in ("tesseract", "ollama running")
    ]
    if not to_install:
        return heal_result(
            True,
            "install_optional_deps",
            "nothing to install (tesseract/ollama are system deps)",
            {},
        )
    try:
        ...
        emit(
            "download_start",
            heal_id="install_optional_deps",
            message=f"pip install: {' '.join(to_install)}",
            details={
                "packages": to_install,
    ...
    try:
        r = subprocess.run(
            [os.getenv("PYTHON", "python"), "-m", "pip", "install", "--quiet"]
            + to_install,
            capture_output=True,
            timeout=120,
            text=True,
        ...
        msg = (
            ...
            if ok
            else (r.stderr or r.stdout or "pip failed")[:200]
        )
        try:
        ...
        try:
            ...
            emit(
                ...
                details={
                    ...
                    "comment_es": _comment_for(to_install, "es"),
                    "comment_en": _comment_for(to_install, "en"),
                    "stderr": (r.stderr or "")[:300] if not ok else None,
                },
            )
        except Exception:
            pass
