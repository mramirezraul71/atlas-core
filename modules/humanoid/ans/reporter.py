...
def write_report(
    ...
    if incidents:
        ...
        for i in incidents[:20]:
            lines.append(
                f"- [{i.get('severity')}] {i.get('check_id')}: {i.get('message')}"
            )
        lines.append("")
    if actions:
...
def get_latest_report() -> str:
    ...
    if not d.exists():
        return ""
    files = sorted(
        d.glob("ANS_REPORT_*.md"), key=lambda x: x.stat().st_mtime, reverse=True
    )
...
def notify_telegram(message: str, severity: str = "medium") -> bool:
    ...
    """
    if os.getenv("ANS_TELEGRAM_NOTIFY", "true").strip().lower() not in (
        "1",
        "true",
        "yes",
    ):
...
def notify_audio(message: str) -> bool:
    ...
    """
    if os.getenv("OPS_AUDIO_ENABLED", "true").strip().lower() not in (
        "1",
        "true",
        "yes",
    ):
...
def notify_all(
    message: str, severity: str = "medium", subsystem: str = "ans"
) -> Dict[str, Any]:
