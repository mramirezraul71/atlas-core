...
def upsert_entity(body: EntityBody):
    try:
        ...
        wm = get_world_model()
        ent = wm.upsert_entity(
            body.name, body.entity_type, body.state, body.properties, body.confidence
        )
...
def create_plan(body: PlanRequest):
    try:
        from modules.humanoid.brain.experience_planner import \
            get_experience_planner

        planner = get_experience_planner()
        plan = planner.plan(
            body.goal, body.context, body.actions if body.actions else None
        )
