...
class EventBus:
    ...
    def subscribe(
        ...
        if self._log_events:
            logger.debug(
                f"Handler subscribed to '{topic}' (priority={priority}, once={once})"
            )
        ...
        with self._lock:
            ...
            for registered_topic, topic_handlers in self._handlers.items():
                ...
                if len(registered_parts) != len(parts):
                    if registered_parts[-1] == "*" and len(registered_parts) <= len(
                        parts
                    ):
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        with self._lock:
                    ...
                    handler_stats.append(
                        {
                            ...
                            "last_error": info.last_error[:100]
                            if info.last_error
                            else None,
                        }
                    )
            ...
            return {
                ...
                "avg_publish_time_ms": self._total_publish_time_ms
                / max(1, self._event_count),
