...
def metalearn_status() -> dict:
    ...
    if not _enabled():
        return _resp(
            ok=True, data={"enabled": False, "error": "METALEARN_ENABLED=false"}, ms=0
        )
    try:
...
def metalearn_run() -> dict:
    ...
    try:
        ...
        return _resp(
            ok=result.get("ok", False), data=result, ms=ms, error=result.get("error")
        )
    except Exception as e:
        ms = int((time.perf_counter() - t0) * 1000)
...
def metalearn_report_latest() -> dict:
    ...
    if not _enabled():
        return _resp(
            ok=True, data={"path": None, "summary": "Meta-learning disabled"}, ms=0
        )
    try:
        ...
        if not path:
            return _resp(
                ok=True, data={"path": None, "summary": "No reports yet"}, ms=0
            )
...
def metalearn_rollback(body: RollbackBody) -> dict:
    ...
    try:
        ...
        return _resp(
            ok=result.get("ok", False), data=result, ms=0, error=result.get("error")
        )
    except Exception as e:
        return _resp(ok=False, data={}, ms=0, error=str(e))
