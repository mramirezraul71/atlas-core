...
def _audit(
    ...
    try:
        ...
        get_audit_logger().log_event(
            "metalearn", "system", "cycle", action, ok, ms, error, payload, None
        )
    except Exception:
        pass
...
def run_cycle(since_ts: Optional[str] = None) -> Dict[str, Any]:
    ...
    try:
        ...
        _audit(
            "metalearn_cycle",
            True,
            {"sample_count": sample_count, "rule_count": rule_count},
            None,
            ms,
        )
        return {
            ...
            "changes_summary": _last_changes_summary
            or train_result.get("changes_summary", ""),
            "error": None,
        }
