...
def _audit(
    ...
    try:
        ...
        get_audit_logger().log_event(
            "orchestrator", "system", module, action, ok, ms, error, payload, None
        )
    except Exception:
        pass
...
def _llm_decompose_fallback(goal: str, fast: bool = True) -> Dict[str, Any]:
    ...
    try:
        ...
        result = call_fn(
            "auto", prompt, use_config=False, prefer_fast=fast, enrich=False
        )
        ...
        if steps and _is_generic_plan(steps):
            _log.warning(
                "Rejected generic plan (%d steps), retrying with strict prompt",
                len(steps),
            )
            strict_prompt = (
                f"Objetivo: {goal.strip()}\n\n"
            ...
            result2 = call_fn(
                "auto", strict_prompt, use_config=False, prefer_fast=False, enrich=False
            )
...
def _decompose(goal: str, fast: bool = True) -> Dict[str, Any]:
    ...
    if not planner:
        ...
        if llm_result and llm_result.get("ok"):
            ...
            _log.info(
                "Decomposed via LLM fallback: %d steps (model=%s)",
                len(steps),
                llm_result.get("model_used", "?"),
            )
...
def _try_heal_step(
    step: Dict[str, Any], step_result: Dict[str, Any], *, goal: str, task_id: str
) -> Dict[str, Any]:
    """
    ...
    try:
        ...
        return {
            ...
            "strategy": res.get("strategy_used"),
        }
    except Exception as e:
...
def run_goal(goal: str, mode: str = "plan_only", fast: bool = True) -> Dict[str, Any]:
    ...
    if not goal or not str(goal).strip():
        ...
        _audit(
            "orchestrator", "run_goal", False, {"task_id": task_id}, "empty goal", ms
        )
        return {
            "ok": False,
            "plan": None,
    ...
    ok_plan, err_plan = validate_plan_structure(
        {"goal": goal, "steps": [s.get("description") for s in steps_data]}
    )
    if not ok_plan:
        ms = int((time.perf_counter() - t0) * 1000)
        return {
    ...
    try:
        ...
        from modules.humanoid.memory_engine import store_plan as mem_store_plan
    ...
    if mode == "plan_only":
        ...
        _audit(
            ...
            "run_goal",
            True,
            {"task_id": task_id, "mode": "plan_only", "steps_count": len(steps_data)},
            None,
            ms,
        )
        return {
    ...
    if mode == "execute_controlled":
        ...
        return {
            ...
            "error": None,
            "ms": ms,
            "message": "Ejecute cada paso vía POST /agent/step/execute con approve=true",
        }
    ...
    while i < len(steps_data):
            ...
            execution_log.append(
                {
                    "step_id": step.get("id"),
                    "status": "skipped",
                    "error": "destructive step blocked",
                }
            )
            i += 1
            continue
        ...
        if step_result.get("status") != "success":
            ...
            if heal.get("attempted"):
                execution_log.append(
                    {"step_id": step.get("id"), "status": "healing", "result": heal}
                )
            if heal.get("success"):
                ...
                if step_result.get("status") == "success" and open_mttr_id is not None:
                    try:
                        from modules.humanoid.ans.mttr import \
                            resolve as mttr_resolve
        ...
        if (
            step_result.get("status") != "success"
            and replans_used < MAX_REPLANS_PER_GOAL
        ):
            replans_used += 1
            re = _replan_goal(
                goal,
                completed=completed_descs,
                last_failed_step=desc,
                last_error=str(step_result.get("error") or ""),
                fast=fast,
            )
            if re.get("ok") and re.get("steps"):
                ...
                plan = {
                    "goal": goal,
                    "steps": steps_data,
                    "raw_steps": re.get("raw_steps", []),
                }
                execution_log.append(
                    {
                        "status": "replan",
                        "replans_used": replans_used,
                        "reason": (step_result.get("error") or "")[:200],
                    }
                )
                save_task(task_id, goal, plan, execution_log, "planned")
                i = 0
    ...
    if open_mttr_id is not None:
        try:
            ...
            mttr_resolve(
                open_mttr_id,
                ok=(
                    len([e for e in execution_log if e.get("status") == "failed"]) == 0
                ),
            )
        except Exception:
            pass
...
def run_goal_with_plan(
    goal: str, steps_list: List[str], mode: str = "plan_only"
) -> Dict[str, Any]:
    ...
    steps_data = [
        ...
        for i, s in enumerate(steps_list or [])
    ]
    ok_plan, err_plan = validate_plan_structure(
        {"goal": goal, "steps": [s.get("description") for s in steps_data]}
    )
    if not ok_plan and steps_data:
        ...
        return {
            ...
            "execution_log": [],
            "artifacts": [],
            "error": err_plan or "invalid steps",
            "ms": ms,
        }
    ...
    if mode == "plan_only":
        ...
        _audit(
            ...
            "run_goal_with_plan",
            True,
            {"task_id": task_id, "mode": "plan_only"},
            None,
            ms,
        )
        return {
    ...
    if mode == "execute_controlled":
        ...
        _audit(
            ...
            "run_goal_with_plan",
            True,
            {"task_id": task_id, "mode": "execute_controlled"},
            None,
            ms,
        )
        return {
    ...
    for i, step in enumerate(steps_data):
        ...
        while step_result.get("status") != "success" and retries < 1:
            ...
            step_result = _execute_step_internal(
                {**step, "description": revised_desc}, task_id
            )
            retries += 1
        execution_log.append(step_result)
        if step_result.get("artifacts"):
    ...
    if thread_id:
        try:
            ...
            add_summary(
                thread_id,
                f"Goal: {goal[:100]}. Steps: {len(steps_data)}, completed: {len([e for e in execution_log if e.get('status') == 'success'])}.",
            )
        except Exception:
            pass
    ms = int((time.perf_counter() - t0) * 1000)
...
def _execute_step_internal(step: Dict[str, Any], task_id: str) -> Dict[str, Any]:
    ...
    actor = ActorContext(
        actor="orchestrator", role=os.getenv("POLICY_DEFAULT_ROLE", "owner")
    )
