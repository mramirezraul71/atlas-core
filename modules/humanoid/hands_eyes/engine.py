...
EVIDENCE_DIR = Path(
    os.getenv("HANDS_EYES_EVIDENCE_DIR", "C:\\ATLAS_PUSH\\snapshots\\hands_eyes")
)
...
def _audit(
    ...
    try:
        ...
        get_audit_logger().log_event(
            "hands_eyes", "system", module, action, ok, 0, None, payload or {}, result
        )
    except Exception:
        pass
...
def capture_full_scene(
    region: Optional[Tuple[int, int, int, int]] = None, save_evidence: bool = True
) -> Dict[str, Any]:
...
def analyze_scene(
    ...
    try:
        ...
        p = (
            prompt
            or "Describe what you see. If UI: list buttons, fields, labels. Suggest actions."
        )
...
def execute_action(
    ...
    try:
        ...
        _audit(
            ...
            r.get("ok", False),
            payload,
            evidence_after or evidence_before,
        )
        return r
    except Exception as e:
        return {
...
def run_workflow(steps: List[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    for step in steps:
        ...
        if kind == "capture":
            ...
            out["steps_done"].append(
                {"kind": "locate", "ok": r.get("ok"), "matches": r.get("matches", [])}
            )
        elif kind == "action":
            r = execute_action(
                step.get("action", ""),
            ...
            out["steps_done"].append(
                {"kind": "action", "ok": r.get("ok"), "action": step.get("action")}
            )
