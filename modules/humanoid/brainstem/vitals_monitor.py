...
class VitalsMonitor:
    ...
    DEFAULT_THRESHOLDS = {
        "battery_percent": VitalThreshold(
            "battery_percent", warning=20, critical=10, is_max=False
        ),
        ...
        "motor_temp": VitalThreshold(
            "motor_temp", warning=60, critical=70, is_max=True
        ),
        "cpu_usage": VitalThreshold("cpu_usage", warning=85, critical=95, is_max=True),
        "memory_usage": VitalThreshold(
            "memory_usage", warning=85, critical=95, is_max=True
        ),
        "disk_usage": VitalThreshold(
            "disk_usage", warning=85, critical=95, is_max=True
        ),
    }

    def __init__(self, update_interval_s: float = 1.0, history_size: int = 3600):
        """
    ...
    def _emit_warning(self, vital_name: str, value: float) -> None:
        ...
        if (
            vital_name not in self._active_alerts
            or self._active_alerts[vital_name].get("level") != "warning"
        ):
    ...
    def _emit_critical(self, vital_name: str, value: float) -> None:
        ...
        if (
            vital_name not in self._active_alerts
            or self._active_alerts[vital_name].get("level") != "critical"
        ):
            self._active_alerts[vital_name] = {
                "level": "critical",
                "value": value,
    ...
    def is_healthy(self) -> bool:
        ...
        return (
            not self._active_alerts
            and not self._current.emergency_stop
            and not self._current.motors_in_fault
            and self._current.battery_percent > 20
            and self._current.sensor_health() > 0.8
        )
    ...
    def set_threshold(
        self, vital_name: str, warning: float = None, critical: float = None
    ) -> None:
