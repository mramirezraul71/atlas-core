...
class GlobalState:
    ...
    ALLOWED_TRANSITIONS = {
        ...
        SystemMode.EMERGENCY: {
            SystemMode.SAFE,
            SystemMode.SHUTDOWN,
        },  # Solo puede ir a SAFE despues de reset
    ...
    async def set_mode(
        ...
        async with self._transition_lock:
            ...
            logger.info(
                f"Mode changed: {old_mode.value} -> {target_mode.value} ({reason})"
            )
    ...
    async def _execute_exit_callbacks(
        self, old_mode: SystemMode, new_mode: SystemMode
    ) -> None:
    ...
    async def _execute_enter_callbacks(
        self, new_mode: SystemMode, old_mode: SystemMode
    ) -> None:
    ...
    def on_enter_mode(
        self, mode: SystemMode, callback: Callable[[SystemMode], None]
    ) -> None:
    ...
    def on_transition(self, callback: Callable[[SystemMode, SystemMode], None]) -> None:
