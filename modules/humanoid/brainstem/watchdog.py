...
class Watchdog:
    ...
    def __init__(
        self, check_interval_s: float = 0.5, max_misses_before_action: int = 3
    ):
        """
    ...
    async def _check_module(self, module: ModuleStatus) -> None:
        ...
        if module.is_overdue():
            ...
            if module.consecutive_misses == 1:
                ...
                logger.warning(
                    f"Module {module.name} missed heartbeat "
                ...
                logger.error(
                    f"Module {module.name} is DEAD "
                    f"(missed {module.consecutive_misses} heartbeats)"
                )
                _bitacora(
                    f"Watchdog: módulo {module.module_id} sin respuesta", ok=False
                )
    ...
    def register_module(
        ...
        is_critical: bool = False,
        auto_restart: bool = True,
        restart_handler: Callable[[], bool] = None,
    ) -> None:
        """
    ...
    def get_critical_dead_modules(self) -> List[ModuleStatus]:
        ...
        return [
            m for m in self._modules.values() if m.status == "dead" and m.is_critical
        ]
