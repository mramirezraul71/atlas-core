...
class Consolidator:
    ...
    def __init__(
        ...
        # Callbacks
        self._on_consolidation_complete: List[
            Callable[[ConsolidationResult], None]
        ] = []

        # Patrones detectados
        self._action_patterns: Dict[str, int] = defaultdict(int)
        self._goal_success_rates: Dict[str, Dict[str, int]] = defaultdict(
    ...
    async def consolidate(self, force: bool = False) -> Optional[ConsolidationResult]:
        ...
        if not force:
            elapsed_hours = (time.time_ns() - self._last_consolidation_ns) / (
                3600 * 1e9
            )
        ...
        logger.info(
            f"Consolidation complete: {episodes_processed} episodes, "
            f"{concepts_created} concepts created, {patterns_found} patterns"
        )

        _bitacora(
            f"Consolidación ejecutada: {episodes_processed} episodios procesados, {concepts_created} conceptos creados, {patterns_found} patrones"
        )
    ...
    def _get_episodes_since_last_consolidation(self) -> List[Episode]:
        ...
        return [
            ep for ep in all_episodes if ep.timestamp_ns > self._last_consolidation_ns
        ]
    ...
    def on_consolidation_complete(
        self, callback: Callable[[ConsolidationResult], None]
    ) -> None:
