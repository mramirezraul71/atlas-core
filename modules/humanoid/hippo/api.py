...
class HippoAPI:
    ...
    async def start_episode(
        self, goal: str, goal_type: str = "", context: Dict[str, Any] = None
    ) -> Episode:
        """
        ...
        Returns:
            Episodio iniciado
        """
        from .schemas import InternalStateSnapshot, WorldStateSnapshot
    ...
    async def add_action_to_episode(
        self, episode: Episode, action_type: str, parameters: Dict[str, Any] = None
    ) -> ActionRecord:
        """
    ...
    async def complete_episode(
        self, episode: Episode, success: bool, reward: float = None
    ) -> str:
        """
    ...
    async def recall_similar_situations(
        self, current_state: Dict[str, Any], threshold: float = 0.7, limit: int = 5
    ) -> List[Episode]:
        """
    ...
    async def query_knowledge(
        self, query: str, relation_type: str = None, limit: int = 10
    ) -> List[Concept]:
        """
    ...
    async def update_knowledge(self, concept: Concept, source: str = "learned") -> str:
        """
    ...
    async def add_knowledge_relation(
        ...
        if not source and create_if_missing:
            source = self.semantic.create_concept(
                source_name, "object", source="inferred"
            )
        if not target and create_if_missing:
            target = self.semantic.create_concept(
                target_name, "object", source="inferred"
            )
    ...
    async def get_related_concepts(
        self, concept_name: str, relation_type: str = None
    ) -> List[Dict[str, Any]]:
        """
    ...
    async def consolidate(self, force: bool = False) -> Optional[ConsolidationResult]:
        ...
        if result:
            _bitacora(
                f"Consolidación ejecutada vía API: {result.episodes_processed} episodios, {result.concepts_created} conceptos"
            )
        return result
