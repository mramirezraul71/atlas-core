...
class EpisodicMemory:
    ...
    def _save_to_disk(self) -> None:
        ...
        try:
            data = {
                ...
                "episodes": [ep.to_full_dict() for ep in self._episodes.values()],
            }
    ...
    def _rebuild_index(self) -> None:
        ...
        try:
            ...
            episodes_with_embeddings = [
                (ep_id, ep) for ep_id, ep in self._episodes.items() if ep.embedding
            ]
            ...
            try:
                ...
                embeddings = np.array(
                    [ep.embedding for _, ep in episodes_with_embeddings],
                    dtype=np.float32,
                )
                ...
                self._episode_ids_index = [
                    ep_id for ep_id, _ in episodes_with_embeddings
                ]

                logger.info(
                    f"FAISS index rebuilt with {len(self._episode_ids_index)} episodes"
                )
    ...
    def _evict_old_episodes(self) -> None:
        ...
        episodes = sorted(
            self._episodes.values(), key=lambda e: (e.importance, e.timestamp_ns)
        )
            ...
            if self._faiss_index is not None:
                try:
                    ...
                    distances, indices = self._faiss_index.search(
                        query_vec, min(limit, len(self._episode_ids_index))
                    )
                    ...
                    if results:
                        _bitacora(
                            f"Recuperación semántica: {len(results)} episodios encontrados para query='{query[:50]}'"
                        )
                    return results
    ...
    def _search_by_keywords(self, query: str, limit: int) -> List[Episode]:
        ...
        for episode in self._episodes.values():
            score = 0
            text = (
                f"{episode.goal} {episode.goal_type} {' '.join(episode.tags)}".lower()
            )
    ...
    def recall_by_goal(self, goal_type: str, limit: int = 10) -> List[Episode]:
        ...
        episodes = [ep for ep in self._episodes.values() if ep.goal_type == goal_type]
        ...
        if episodes:
            _bitacora(
                f"Recuperación por objetivo: {len(episodes[:limit])} episodios tipo={goal_type}"
            )
    ...
    def recall_successful(self, limit: int = 10) -> List[Episode]:
        ...
        episodes = [
            ep for ep in self._episodes.values() if ep.outcome == Outcome.SUCCESS
        ]
    ...
    def recall_failures(self, limit: int = 10) -> List[Episode]:
        ...
        episodes = [
            ep for ep in self._episodes.values() if ep.outcome == Outcome.FAILURE
        ]
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "average_reward": total_reward / len(self._episodes)
            if self._episodes
            else 0,
