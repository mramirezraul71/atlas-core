...
def _audit(
    ...
    try:
        ...
        get_audit_logger().log_event(
            "agents", "system", module, action, ok, ms, error, payload, None
        )
    except Exception:
        pass
...
def _store_in_memory(
    goal: str, plan: Dict[str, Any], task_id: str, thread_id: Optional[str]
) -> None:
    try:
...
class ExecutiveAgent(BaseAgent):
    ...
    def process(self, context: Dict[str, Any]) -> Dict[str, Any]:
        ...
        plan = {
            "goal": goal,
            "steps": [
                {"id": f"step_{i+1}", "description": s} for i, s in enumerate(steps)
            ],
            "raw_steps": steps,
        }
        return {
    ...
    def _run_chain(
        self, reg: Any, ctx: Dict[str, Any], depth: int = 1
    ) -> Dict[str, Any]:
        engineer = reg.get("engineer")
        if engineer:
            r = engineer.run(ctx)
...
def run_multi_agent_goal(
    goal: str, depth: int = 2, mode: str = "plan_only"
) -> Dict[str, Any]:
    """
    ...
    if not executive:
        ...
        _audit(
            "executive", "run_goal", False, None, "executive agent not registered", ms
        )
        return {
            "ok": False,
            "task_id": task_id,
    ...
    if not out.get("ok"):
        _audit(
            "executive", "run_goal", False, {"task_id": task_id}, out.get("error"), ms
        )
        return {
            "ok": False,
            "task_id": task_id,
    ...
    _audit(
        ...
        "run_goal",
        True,
        {"task_id": task_id, "depth": depth, "decision": output.get("decision")},
        None,
        ms,
    )
    return {
