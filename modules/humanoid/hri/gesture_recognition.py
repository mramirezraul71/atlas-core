...
class GestureRecognizer:
    ...
    def process_landmarks(
        self, landmarks: np.ndarray, hand: str = "right"
    ) -> Optional[Gesture]:
        """
        ...
        fingers_closed = all(
            landmarks[tip][1] > landmarks[tip - 2][1] for tip in [8, 12, 16, 20]
        )
        ...
        fingers_extended = all(
            np.linalg.norm(landmarks[tip] - wrist) > 0.15 for tip in [4, 8, 12, 16, 20]
        )
        if fingers_extended:
            return Gesture(
        ...
        others_closed = all(
            np.linalg.norm(landmarks[tip] - wrist) < 0.12 for tip in [12, 16, 20]
        )
    ...
    def detect_wave(self, history_window_seconds: float = 2.0) -> bool:
        ...
        recent = [
            g for t, g in self._gesture_history if now - t < history_window_seconds
        ]
