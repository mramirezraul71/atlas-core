...
class STTEngine:
    ...
    def _init_engine(self):
        try:
            ...
            _log.info(
                f"Micrófono calibrado. Umbral de energía: {self._recognizer.energy_threshold}"
            )
        except Exception as e:
...
class CommandProcessor:
    ...
    def process(self, text: str) -> str:
        ...
        if any(
            w in text_lower
            for w in ["hola", "buenos días", "buenas tardes", "buenas noches"]
        ):
        ...
        if any(
            w in text_lower for w in ["cómo estás", "cómo te encuentras", "qué tal"]
        ):
            return "Estoy funcionando perfectamente. Todos mis sistemas están operativos. ¿En qué puedo ayudarte?"

        if any(
            w in text_lower for w in ["quién eres", "qué eres", "cuál es tu nombre"]
        ):
        ...
        if any(
            w in text_lower
            for w in ["estado", "status", "diagnóstico", "cómo está el sistema"]
        ):
    ...
    def _ask_brain(self, text: str) -> str:
        ...
        try:
            ...
            payload = json.dumps(
                {"message": text, "context": "voice_interaction", "max_tokens": 150}
            ).encode()
...
class VoiceAssistant:
    ...
    def _set_state(self, state: AssistantState, info: str = ""):
        ...
        try:
            print(
                f"\r{icon} [{state.value.upper()}] {info[:60]:<60}", end="", flush=True
            )
        except Exception:
            pass
...
def main():
    ...
    parser.add_argument(
        "--rate", type=int, default=150, help="Velocidad TTS (default: 150)"
    )
    ...
    config = VoiceConfig(debug=args.debug, use_llm=not args.no_llm, tts_rate=args.rate)
