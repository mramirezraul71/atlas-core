...
def _mode_allows() -> bool:
    try:
        from modules.humanoid.mode.config import (is_observe_only,
                                                  is_selfprog_enabled)
...
def create_module(
    name: str, base_path: Optional[str] = None, template: str = "module"
) -> Dict[str, Any]:
    ...
    base = (
        base_path
        or _env_str("POLICY_ALLOWED_PATHS", "C:\\ATLAS_PUSH").split(",")[0].strip()
    )
...
def create_endpoint(
    route: str, method: str = "GET", handler: str = ""
) -> Dict[str, Any]:
    ...
    try:
        ...
        record_evolution(
            "create_endpoint",
            goal=route,
            spec_json={"method": method, "handler": handler},
            outcome="registered",
            ok=True,
        )
    except ImportError:
...
def add_skill(name: str, handler_path: str = "") -> Dict[str, Any]:
    ...
    try:
        ...
        record_evolution(
            "add_skill",
            goal=name,
            spec_json={"handler_path": handler_path},
            outcome="registered",
            ok=True,
        )
    except ImportError:
...
def run_selfprog_flow(
    ...
    if apply and spec.get("run_smoke", True):
        ...
        try:
            from modules.humanoid.mode.config import (is_rollback_enabled,
