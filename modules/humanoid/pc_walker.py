...
def run_workflow(
    ...
    try:
        for i, st in enumerate(steps or []):
            ...
            if _is_destructive_step(st) and not allow_destructive:
                ...
                _ops("pc_walk", msg, level="med", data={"i": i, "kind": kind})
                    ...
                    {"i": i, "ok": False, "kind": kind, "error": "destructive_blocked"}
                )
                return WalkResult(
                    ok=False,
                    steps=done,
            ...
            if kind in ("hotkey",):
                ...
                _ops(
                    ...
                    level="info" if r.get("ok") else "med",
                    data={"i": i, "keys": keys, "timeout": err == "timeout"},
                )
                continue
            ...
            if kind in ("type",):
                ...
                _ops(
                    "pc_walk",
                    f"Type len={len(text)} => {'OK' if r.get('ok') else 'FAIL'}",
                    level="info" if r.get("ok") else "med",
                    data={"i": i, "len": len(text), "timeout": err == "timeout"},
                )
                continue
            ...
            if kind in ("click_xy", "click"):
                ...
                _ops(
                    "pc_walk",
                    f"Click ({x},{y}) => {'OK' if r.get('ok') else 'FAIL'}",
                    level="info" if r.get("ok") else "med",
                    data={"i": i, "x": x, "y": y, "timeout": err == "timeout"},
                )
                continue
            ...
            if kind == "click_text":
                ...
                if not loc.get("ok") or not matches:
                    done.append(
                        {
                            ...
                            "kind": "click_text",
                            "query": q,
                            "error": loc.get("error") or "not_found",
                        }
                    )
                    _ops(
                        "pc_walk",
                        f"Locate '{q}' => NOT FOUND",
                        level="med",
                        data={"i": i, "query": q},
                    )
                    continue
                ...
                _ops(
                    "pc_walk",
                    f"ClickText '{q}' ({cx},{cy}) => {'OK' if r.get('ok') else 'FAIL'}",
                    level="info" if r.get("ok") else "med",
                    data={
                        "i": i,
                        "query": q,
            ...
            if kind == "screenshot":
                ...
                if path:
                    last_ev = path
                        ...
                        {"i": i, "ok": True, "kind": "screenshot", "path": path}
                    )
                    _ops(
                        "pc_walk",
                        f"Screenshot guardado: {path}",
                        level="info",
                        data={"i": i},
                        evidence_path=path,
                    )
                    ...
                    done.append(
                        {
                            ...
                            "ok": False,
                            "kind": "screenshot",
                            "error": (cerr or err or "capture_failed"),
                        }
                    )
                    _ops(
                        "pc_walk",
                        f"Screenshot FAIL: {cerr or err}",
                        level="med",
                        data={"i": i, "timeout": err == "timeout"},
                    )
                continue
            ...
            if kind == "ocr":
                ...
                try:
                    ...
                    _ops(
                        "pc_walk",
                        f"OCR chars={len(text)} err={oerr or 'none'}",
                        level="info" if ok else "med",
                        data={"i": i, "chars": len(text)},
                        evidence_path=path,
                    )
                except Exception as e:
                    done.append(
            ...
            if kind == "alt_tab":
                ...
                for _ in range(max(1, min(10, count))):
                    ok, _, err = _worker_call(
                        "screen_act",
                        {"action": "hotkey", "payload": {"keys": ["alt", "tab"]}},
                        step_timeout_s,
                    )
            ...
            if kind == "open_app":
                ...
                _ops(
                    "pc_walk",
                    f"Abrir app (shell): {name} => {'OK' if r.get('ok') else 'FAIL'}",
                    level="info" if r.get("ok") else "med",
                    data={"i": i, "name": name},
                )
                continue
            ...
            if kind == "run_dialog":
                ...
                ok, _, err = _worker_call(
                    "screen_act",
                    {"action": "hotkey", "payload": {"keys": ["winleft", "r"]}},
                    step_timeout_s,
                )
                if not ok and err == "timeout":
                    _disabled_until_ts = time.time() + 60
                ...
                _worker_call(
                    "screen_act",
                    {"action": "type", "payload": {"text": cmd}},
                    step_timeout_s,
                )
                time.sleep(0.1)
                _worker_call(
                    "screen_act",
                    {"action": "hotkey", "payload": {"keys": ["enter"]}},
                    step_timeout_s,
                )
            ...
            if kind == "open_path":
                ...
                _ops(
                    "pc_walk",
                    f"Abrir ruta (shell): {path} => {'OK' if r.get('ok') else 'FAIL'}",
                    level="info" if r.get("ok") else "med",
                    data={"i": i, "path": path},
                )
                continue
                ...
                {"i": i, "ok": False, "kind": kind, "error": "unsupported_kind"}
            )
            _ops(
                "pc_walk",
                f"Paso no soportado: {kind}",
                level="med",
                data={"i": i, "kind": kind},
            )
        ...
        return WalkResult(
            ok=ok_all, steps=done, ms=ms, error=None, last_evidence=last_ev
        ).__dict__
    except Exception as e:
        ms = int((time.perf_counter() - t0) * 1000)
        return WalkResult(
            ok=False, steps=done, ms=ms, error=str(e), last_evidence=last_ev
        ).__dict__
