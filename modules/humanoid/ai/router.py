...
def infer_task_profile(
    prompt: str, intent_hint: Optional[str] = None, modality: str = "text"
) -> TaskProfile:
...
def _profile_to_route(profile: TaskProfile) -> str:
    ...
    if profile.modality == "image":
        return "VISION"
    if profile.latency_need == "fast":
        return "FAST"
    return "CHAT"


def _call_ollama(
    model_name: str, prompt: str, system: Optional[str], timeout_s: int
...
def route_and_run(
    ...
    try:
        ...
        if full_key and ":" in full_key:
            ...
            _timeout = (
                ...
                else int(os.getenv("OLLAMA_TIMEOUT_S", "60") or "60")
            )
            if provider_id == "ollama" and ollama_ok:
                ...
                telemetry_record(
                    ...
                    ok,
                    0.0,
                    "" if ok and out else "empty_response",
                )
                if ok and out:
                ...
                provider_id in ("openai", "anthropic", "gemini", "perplexity")
                and policies.paid_api_allowed()
            ):
                api_key = get_provider_api_key(provider_id)
                if api_key:
                    ok, out, ms = call_external(
                        provider_id, model_name, prompt, system, api_key, _timeout
                    )
                    meta["latency_ms"] = ms
                    telemetry_record(
                        provider_id,
    ...
    for spec in chain:
        ...
        if spec.provider_id in ("openai", "anthropic", "gemini", "perplexity"):
            ...
            aid = propose_paid_upgrade(
                "quality_insufficient_or_fallback",
                decision.cost_estimate_usd or 0.1,
                route,
            )
