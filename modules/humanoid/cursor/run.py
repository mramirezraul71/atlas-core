...
def _capabilities_steps() -> List[Dict[str, Any]]:
    return [
        {
            "description": "Diagnóstico rápido: /status, /health, ANS, métricas, scheduler, governance.",
            "status": "pending",
        },
        {
            "description": "Planificación tipo Cursor: generar pasos para un objetivo y ejecutar en modo controlado/auto.",
            "status": "pending",
        },
        {
            "description": "OPS (comunicación permanente): audio PC + Telegram (y WhatsApp si hay credenciales) con evidencia.",
            "status": "pending",
        },
        {
            "description": "Web (Pies digitales): abrir URLs, click/fill, extraer texto y screenshots (Playwright).",
            "status": "pending",
        },
        {
            "description": "Visión (Ojos): snapshot/stream del robot con enhance, zoom y foco digital.",
            "status": "pending",
        },
        {
            "description": "Manos (Input local): click/teclado/hotkeys con governance + rate-limit + OPS.",
            "status": "pending",
        },
        {
            "description": "Owner Console: aprobaciones, sesiones, Emergency Stop y cadena de integridad.",
            "status": "pending",
        },
    ]
...
def cursor_run(
    ...
    effective_depth = (
        _resources_to_depth(resources) if resources else max(1, min(5, depth))
    )
    ...
    try:
        ...
        try:
            ...
            if (
                (decision.provider_id or "").strip().lower() == "null"
                or raw.startswith("[NullProvider]")
                or "System:" in raw
            ):
                ...
                for line in raw.split("\n"):
                    ...
                    if line.startswith("[") and (
                        "NullProvider" in line or "Último error" in line
                    ):
                        continue
        ...
        if mode == "plan_only":
            next_actions = [
                "Ejecuta un paso con POST /cursor/step/execute",
                "O usa modo=controlled/auto para ejecución",
            ]
        elif mode == "auto":
            ...
            needs = sum(
                1
                for x in auto_result.get("executed", [])
                if x.get("action") == "needs_human"
            )
            ...
            next_actions = [
                "Modo controlled: usa /cursor/step/execute (approve=true) para ejecutar un paso y ver terminal."
            ]
