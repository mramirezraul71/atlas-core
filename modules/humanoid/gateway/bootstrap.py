...
HQ_BASE_URL = os.getenv(
    "ATLAS_HQ_URL", ""
).strip()  # HQ URL for register call (from worker perspective often same host)


def _register_worker_at_hq(
    hq_base_url: str,
    node_id: str,
    base_url: str,
...
def bootstrap(
    ...
    if mode == "auto":
        for name in selector._order():
            ...
            if not target:
                continue
            tried.append(name)
            gateway_audit.log_gateway_event(
                "bootstrap_attempt",
                False,
                {"mode": name, "target": target.base_url},
                None,
                0,
            )
            ...
            if probed.get("ok"):
                ...
                if reg.get("ok"):
                    ...
                    return {
                        ...
                        "score": probed.get("score"),
                        "ms": ms,
                        "registered": True,
                    }
                gateway_audit.log_gateway_event(
                    "gateway_fail", False, {"mode": name}, reg.get("error"), ms
                )
                recommendations.append(
                    f"Register failed for {name}: {reg.get('error')}"
                )
            else:
        ...
        gateway_audit.log_gateway_event(
            "gateway_fail",
            False,
            {"tried": tried, "recommendations": recommendations},
            "all modes failed",
            ms,
        )
        return {
    ...
    if not target:
        ms = int((time.perf_counter() - t0) * 1000)
        gateway_audit.log_gateway_event(
            "gateway_fail", False, {"mode": mode}, "no_gateway_config", ms
        )
        return {"ok": False, "error": "no_gateway_config", "mode": mode, "ms": ms}
    gateway_audit.log_gateway_event(
        "bootstrap_attempt", False, {"mode": mode, "target": target.base_url}, None, 0
    )
    probed = health.probe_worker_with_retries(target.base_url)
    ms = int((time.perf_counter() - t0) * 1000)
    if not probed.get("ok"):
        gateway_audit.log_gateway_event(
            "gateway_fail", False, {"mode": mode}, probed.get("error"), ms
        )
        return {
            "ok": False,
            ...
            "mode": mode,
            "ms": ms,
            "recommendations": [probed.get("error", "")],
        }
    reg = _register_worker_at_hq(hq, nid, target.base_url, capabilities)
    if reg.get("ok"):
        store.set_last_success(mode, target.base_url)
        gateway_audit.log_gateway_event(
            "gateway_success", True, {"mode": mode, "target": target.base_url}, None, ms
        )
        return {
            "ok": True,
            "mode": mode,
    ...
    gateway_audit.log_gateway_event(
        "gateway_fail", False, {"mode": mode}, reg.get("error"), ms
    )
    return {
        "ok": False,
        "error": reg.get("error", "register failed"),
        "mode": mode,
        "ms": ms,
    }
