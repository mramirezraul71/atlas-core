...
def _tailscale_path() -> Optional[Path]:
    ...
    try:
        r = subprocess.run(
            ["where", "tailscale"], capture_output=True, text=True, timeout=5
        )
    ...
    try:
        ...
        if r.returncode != 0:
            return {
                "ok": False,
                "available": True,
                "error": r.stderr or "status failed",
            }
        import json
...
def ping(node: str) -> Dict[str, Any]:
    ...
    try:
        ...
        return {
            "ok": ok,
            "latency_ms": latency,
            "error": None if ok else (r.stderr or r.stdout or "ping failed"),
        }
    except Exception as e:
...
def get_worker_url() -> Optional[str]:
    ...
    dns = (
        os.getenv("TAILSCALE_DNS_NAME", "").strip()
        or os.getenv("TAILSCALE_EXPECTED_NODE", "").strip()
    )
    if not dns:
        return None
