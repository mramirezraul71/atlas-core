...
def diagnose_with_self_model(
    incidents: List[Dict], actions: List[Dict], manifest: Optional[Dict] = None
) -> Dict[str, Any]:
    """
    ...
    for inc in incidents:
        ...
        if cid not in diagnosis["incidents_by_check"]:
            diagnosis["incidents_by_check"][cid] = {
                "incident": inc,
        ...
        for a in actions_taken:
            ...
            if hid and hid != "(omitido)":
                ...
                diagnosis["incidents_by_check"][cid]["blocked"].append(
                    a.get("message", "")
                )
        ...
        for hid in suggested:
            if hid in heals and heals[hid].get("safe"):
                if not any(
                    t.get("heal_id") == hid
                    for t in diagnosis["incidents_by_check"][cid]["tried"]
                ):
                    diagnosis["heals_available"].append({"check": cid, "heal": hid})
            else:
                diagnosis["heals_blocked"].append(
                    {
                        "check": cid,
                        "heal": hid,
                        "reason": "no safe"
                        if hid not in heals
                        else "not in SAFE_HEALS",
                    }
                )
    ...
    if diagnosis["root_causes"]:
        diagnosis["recommendations"].append(
            "Incidentes sin heal disponible: considerar añadir heals o corrección manual."
        )
    ...
    diagnosis[
        "summary"
    ] = f"{issues} incidentes, {acted} acciones. Causas raíz: {len(diagnosis['root_causes'])}."
