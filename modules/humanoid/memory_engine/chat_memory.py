...
class ChatMemoryEngine:
    ...
    def create_session(self, user_id: str = None, context: Dict[str, Any] = None) -> str:
        """
        ...
        try:
            ...
            for conv in conversations:
                ...
                by_session[session_id]["max_relevance"] = max(
                    by_session[session_id]["max_relevance"],
                    conv["relevance_score"]
                )
    ...
    def get_context_for_new_message(
        ...
        try:
            ...
            search_result = self.search_conversations(
                query=new_message,
                limit=5
            )
            ...
            if search_result["success"]:
                for session in search_result["results"]:
                    if session["session_id"] == session_id:
                        relevant_context.extend(session["messages"][:2])  # Top 2 por sesión
    ...
    def _load_session(self, session_id: str):
        ...
        try:
            ...
            if results:
                for result in results:
                    metadata = result["metadata"]
                    if metadata.get("session_id") == session_id and metadata.get("type") == "session_info":
    ...
    def _create_summary(self, session_id: str):
        ...
        try:
            ...
            if not history["success"] or len(history["messages"]) < self.summary_threshold:
                return
            ...
            print(f"✅ Resumen creado para sesión {session_id} ({len(messages)} mensajes)")
    ...
    def _generate_simple_summary(self, messages: List[Dict]) -> str:
        ...
        keywords = ["implementar", "desarrollar", "crear", "solucionar", "analizar", "diseñar"]
...
if __name__ == "__main__":
    ...
    chat_engine.add_message(session_id, "Claro, ¿qué necesitas saber sobre ChromaDB?", "assistant")
