...
class LifeLog:
    ...
    def log_perception(
        self, source: str, perception: str, data: Dict = None, **kw
    ) -> str:
        return self.log(
            "perception", source, perception=perception, perception_data=data, **kw
        )
    ...
    def log_feedback(
        self, source: str, feedback: str, sentiment: float = 0.0, **kw
    ) -> str:
        return self.log(
            "feedback",
            source,
    ...
    def get_stats(self) -> Dict[str, Any]:
        with _con() as c:
            ...
            by_type = {}
            for r in c.execute(
                "SELECT event_type, COUNT(*) as n FROM lifelog GROUP BY event_type"
            ).fetchall():
                by_type[r["event_type"]] = r["n"]
            by_source = {}
            for r in c.execute(
                "SELECT source, COUNT(*) as n FROM lifelog GROUP BY source ORDER BY n DESC LIMIT 10"
            ).fetchall():
                by_source[r["source"]] = r["n"]
            success_count = c.execute(
                "SELECT COUNT(*) FROM lifelog WHERE success = 1"
            ).fetchone()[0]
            fail_count = c.execute(
                "SELECT COUNT(*) FROM lifelog WHERE success = 0"
            ).fetchone()[0]
            sessions = c.execute("SELECT COUNT(*) FROM lifelog_sessions").fetchone()[0]
            feedback_count = c.execute(
                "SELECT COUNT(*) FROM lifelog WHERE human_feedback != ''"
            ).fetchone()[0]
