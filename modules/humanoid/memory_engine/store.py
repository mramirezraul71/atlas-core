...
def _fts_insert(
    thread_id: Optional[str], task_id: Optional[str], kind: str, content: str
) -> None:
...
def ensure_thread(thread_id: Optional[str] = None, title: str = "") -> str:
    ...
    conn = _ensure()
    conn.execute(
        "INSERT INTO threads (id, title, created_ts, updated_ts) VALUES (?, ?, ?, ?)",
        (tid, title or "default", now, now),
    )
    conn.commit()
    return tid
    ...
    try:
        if kind == "artifact":
            ...
            aid = store_artifact(
                task_id,
                run_id,
                payload.get("artifact_kind", "write"),
                path_or_uri,
                preview,
            )
            return {
        ...
        if kind == "decision":
            did = store_decision(
                thread_id, task_id, payload.get("decision_type", "api"), payload
            )
            return {
                "ok": True,
                "id": did,
...
def store_run(
    ...
    _fts_insert(
        ...
        task_id,
        "run",
        (error or "") + (json.dumps(result) if result else "")[:2000],
    )
    return run_id
...
def store_decision(
    ...
    row = conn.execute("SELECT last_insert_rowid()").fetchone()
    _fts_insert(
        thread_id, task_id, "decision", decision_type + " " + json.dumps(payload)[:1000]
    )
    return row[0] if row else 0


def search_similar_plans(
    query: str, top_k: int = 5, min_similarity: float = 0.6
) -> List[Dict[str, Any]]:
...
def add_summary(thread_id: str, content: str) -> int:
    ...
    conn = _ensure()
    conn.execute(
        "INSERT INTO summaries (thread_id, content, created_ts) VALUES (?, ?, ?)",
        (thread_id, content[:50000], now),
    )
    conn.commit()
    row = conn.execute("SELECT last_insert_rowid()").fetchone()
