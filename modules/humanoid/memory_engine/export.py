...
def export_markdown(thread_id: str = "", task_id: str = "", limit: int = 100) -> str:
    ...
    if task_id:
        row = conn.execute(
            "SELECT id, thread_id, goal, plan_json, status, created_ts FROM tasks WHERE id = ?",
            (task_id,),
        ).fetchone()
        if row:
            ...
            for r in conn.execute(
                "SELECT step_id, ok, error, result_json, created_ts FROM runs WHERE task_id = ? ORDER BY id",
                (task_id,),
            ).fetchall():
                lines.append(f"- Run {r[0]}: ok={r[1]} error={r[2] or '-'} at {r[4]}")
            for a in conn.execute(
                "SELECT kind, path_or_uri, created_ts FROM artifacts WHERE task_id = ?",
                (task_id,),
            ).fetchall():
        ...
        for row in conn.execute(
            "SELECT id, thread_id, goal, status, created_ts FROM tasks ORDER BY created_ts DESC LIMIT ?",
            (limit,),
        ).fetchall():
            lines.append(
                f"- Task {row[0]} (thread {row[1]}): {row[2][:60]} - {row[3]} at {row[4]}"
            )
    return "\n".join(lines)
