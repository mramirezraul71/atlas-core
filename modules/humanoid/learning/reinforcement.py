...
class Policy:
    ...
    def get_q_values(self, state: Dict[str, float]) -> Dict[str, float]:
        ...
        if state_key not in self._q_table:
            ...
            self._q_table[state_key] = {
                action: random.uniform(-0.1, 0.1) for action in self.action_space
            }

        return self._q_table[state_key]

    def select_action(self, state: Dict[str, float], explore: bool = True) -> str:
        """
    ...
    def decay_epsilon(
        self, min_epsilon: float = 0.01, decay_rate: float = 0.995
    ) -> None:
...
class ReinforcementLearning:
    ...
    def create_policy(self, task_id: str, action_space: List[str], **kwargs) -> Policy:
        """
        ...
        logger.info(
            f"Created policy for task {task_id} with {len(action_space)} actions"
        )
        return policy
    ...
    def set_reward_function(
        self, task_id: str, reward_fn: Callable[[Dict, str, Dict], float]
    ) -> None:
        """
    ...
    def compute_reward(
        ...
        if task_id in self._reward_functions:
            try:
                custom_reward = self._reward_functions[task_id](
    ...
    async def train_batch(self, task_id: str) -> float:
        ...
        for exp in batch:
            action = (
                exp.action.get("action") if isinstance(exp.action, dict) else exp.action
            )
            error = policy.update(
                exp.state, action, exp.reward, exp.next_state, exp.done
            )
    ...
    def get_episode_stats(self, task_id: str, last_n: int = 100) -> Dict[str, Any]:
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "buffer_sizes": {
                task_id: len(buf) for task_id, buf in self._buffers.items()
            },
        }
