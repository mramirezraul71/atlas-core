...
class PathController:
    ...
    def compute_velocity(self, pose: Tuple[float, float, float]) -> ControlCommand:
        ...
        angular_vel = max(
            -self.config.max_angular_vel, min(self.config.max_angular_vel, angular_vel)
        )
...
class DWAController:
    ...
    def compute_velocity(
        ...
        v_max = min(
            self.config.max_linear_vel, self._current_vel[0] + self.max_accel * dt
        )
        w_min = max(
            -self.config.max_angular_vel,
            self._current_vel[1] - self.max_angular_accel * dt,
        )
        w_max = min(
            self.config.max_angular_vel,
            self._current_vel[1] + self.max_angular_accel * dt,
        ...
        for v in [v_min + i * (v_max - v_min) / 10 for i in range(11)]:
            for w in [w_min + i * (w_max - w_min) / 10 for i in range(11)]:
                ...
                dist_score = 1.0 / (
                    1.0
                    + math.sqrt(
                        (self._goal[0] - sim_x) ** 2 + (self._goal[1] - sim_y) ** 2
                    )
                )
