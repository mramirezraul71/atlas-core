...
class NavigationSystem:
    ...
    def goto(self, x: float, y: float, theta: Optional[float] = None) -> Dict[str, Any]:
        ...
        with self._lock:
            if self._state not in (
                ...
                return {
                    "ok": False,
                    "error": f"Cannot navigate in state {self._state.value}",
                }
            ...
            _log.info(
                "Navigating to (%.2f, %.2f), path length: %.2fm", x, y, path.length
            )
            return {
                "ok": True,
    ...
    def cancel_navigation(self) -> Dict[str, Any]:
        ...
        with self._lock:
            ...
            if self._state == NavigationState.NAVIGATING:
                self._state = (
                    NavigationState.LOCALIZING
                    if self._slam.mode == SLAMMode.LOCALIZATION
    ...
    def process_odometry(
        ...
        odom = Odometry(
            x=x, y=y, theta=theta, vx=vx, vy=vy, vtheta=vtheta, timestamp=time.time()
        )
    ...
    def get_status(self) -> Dict[str, Any]:
        ...
        with self._lock:
            ...
            return {
                ...
                "path_progress": self._controller.get_progress()
