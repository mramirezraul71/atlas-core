...
class PathPlanner:
    ...
    def set_costmap(
        self, costmap: np.ndarray, resolution: float, origin: Tuple[float, float]
    ):
    ...
    def _inflate_costmap(self):
        ...
        inflation_cells = int(
            (self._inflation_radius + self._robot_radius) / self._resolution
        )
        ...
        for y, x in zip(occupied[0], occupied[1]):
            for dy in range(-inflation_cells, inflation_cells + 1):
                for dx in range(-inflation_cells, inflation_cells + 1):
                    ny, nx = y + dy, x + dx
                    if (
                        0 <= ny < self._costmap.shape[0]
                        and 0 <= nx < self._costmap.shape[1]
                    ):
                        ...
                        if dist < self._inflation_radius + self._robot_radius:
                            ...
                            cost = int(
                                ...
                                * (
                                    1
                                    - dist
                                    / (self._inflation_radius + self._robot_radius)
                                )
                            )
    ...
    def plan(
        ...
        waypoints = [
            Waypoint(
                x=self._map_to_world(c[0], c[1])[0], y=self._map_to_world(c[0], c[1])[1]
            )
            for c in path_cells
        ]
    ...
    def _rrt(
        self, start: Tuple[int, int], goal: Tuple[int, int], max_iter: int = 5000
    ) -> Tuple[Optional[List[Tuple[int, int]]], float]:
    ...
    def _simplify_path(
        self, waypoints: List[Waypoint], tolerance: float = 0.1
    ) -> List[Waypoint]:
    ...
    def _point_line_distance(
        self, point: Waypoint, line_start: Waypoint, line_end: Waypoint
    ) -> float:
