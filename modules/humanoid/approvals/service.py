...
def _notify_telegram_approval_pending(item: Dict[str, Any]) -> None:
    """If Telegram enabled, send notification for HIGH pending approval. No-op if deps missing."""
    ...
    try:
        ...
        msg = f"[ATLAS] Approval pendiente HIGH: id={item.get('id')} action={item.get('action')}. Revisa /ui o POST /approvals/approve"
...
def create(action: str, payload: Dict[str, Any], job_id: Optional[str] = None, run_id: Optional[int] = None) -> Dict[str, Any]:
    ...
    try:
        ...
        item = store_create(action=action, payload=payload, risk=risk, job_id=job_id, run_id=run_id)
        try:
...
def approve(aid: str, resolved_by: str = "api") -> Dict[str, Any]:
    ok = store_approve(aid, resolved_by=resolved_by)
    try:
        from modules.humanoid.audit import get_audit_logger
