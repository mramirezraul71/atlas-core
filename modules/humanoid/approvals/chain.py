...
def _secret() -> bytes:
    s = (
        os.getenv("APPROVALS_CHAIN_SECRET")
        or os.getenv("APPROVALS_CHAIN_SECRET", "CHANGE_ME_LONG_RANDOM")
        or ""
    ).strip()
...
def enabled() -> bool:
    return os.getenv("APPROVALS_CHAIN_ENABLED", "true").strip().lower() in (
        "1",
        "true",
        "yes",
    )
...
def compute_chain_hash(
    ...
    """
    data = (
        f"{prev_hash}|{approval_id}|{status}|{request_hash}|{approved_ts}|{approved_by}"
    )
...
def verify_chain(items: List[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    for item in items:
        ...
        expected = compute_chain_hash(
            prev, aid, status, req_hash, resolved, resolved_by
        )
