...
class Inhibitor:
        ...
        self.add_rule(
            InhibitionRule(
                ...
                condition=lambda a, c: (
                    c.get("nearest_human_distance", float("inf")) < 0.3
                    and getattr(a, "action_type", None) in ("move", "grasp", "navigate")
                ),
            )
        )
    ...
    def add_learned_inhibition(
        ...
        rule = InhibitionRule(
            ...
            condition=lambda a, c, p=pattern_lower: (
                p in str(getattr(a, "description", "")).lower()
                or p in str(getattr(a, "action_type", "")).lower()
            ),
    ...
    def add_user_prohibition(
        ...
        rule = InhibitionRule(
            ...
            condition=lambda a, c, p=pattern_lower: (
                p in str(getattr(a, "description", "")).lower()
                or p in str(getattr(a, "id", "")).lower()
            ),
        )
