...
class ActionSelector:
    ...
    def __init__(
        ...
        self._utility_evaluators: List[
            Callable[[ActionCandidate, SelectionContext], float]
        ] = []
        self._risk_evaluators: List[
            Callable[[ActionCandidate, SelectionContext], float]
        ] = []
    ...
    def select(
        ...
        if selected:
            _bitacora(
                f"Acción seleccionada: {selected.action_type} conf={selected.confidence:.2f}"
            )
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "exploration_rate": self._exploration_count
            / max(1, self._total_selections),
            "avg_candidates": sum(
                len(r.all_candidates) for r in self._selection_history[-100:]
            )
            / min(100, len(self._selection_history)),
            "avg_selection_time_ms": sum(
                r.selection_time_ms for r in self._selection_history[-100:]
            )
            / min(100, len(self._selection_history)),
        ...
        return [
            {
                ...
                "type": r.selected_action.action_type.value
