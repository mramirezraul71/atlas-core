...
class Waypoint:
    ...
    def interpolate(self, other: "Waypoint", t: float) -> "Waypoint":
        ...
        new_time = self.time_from_start + t * (
            other.time_from_start - self.time_from_start
        )
        ...
        if self.joints and other.joints:
            ...
            for key in self.joints:
                if key in other.joints:
                    new_joints[key] = self.joints[key] + t * (
                        other.joints[key] - self.joints[key]
                    )
...
class Trajectory:
    ...
    def sample(self, t: float) -> Optional[Waypoint]:
        ...
        for i in range(len(self.waypoints) - 1):
            if (
                self.waypoints[i].time_from_start
                <= t
                <= self.waypoints[i + 1].time_from_start
            ):
