...
class PIDController:
    ...
    def compute(self, setpoint: float, measurement: float) -> float:
        ...
        self._integral = max(
            -self.gains.integral_limit, min(self.gains.integral_limit, self._integral)
        )
...
class MotorController:
    ...
    def add_joint(self, config: JointConfig) -> None:
        ...
        self._impedance_controllers[config.id] = ImpedanceController(
            config.impedance_params
        )
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        avg_time = self._total_control_time_ns / max(1, self._loop_count) / 1e6
        return {
