...
from .motor_controller import ControlMode, JointConfig, MotorController
from .trajectory_planner import (Pose3D, Trajectory, TrajectoryPlanner,
                                 TrajectoryType)
...
class MotorInterface:
    ...
    async def execute(self, command: HighLevelCommand) -> ExecutionResult:
        ...
        target = (
            ...
            or command.parameters.get("pose")
            or command.parameters.get("object_pose")
        )
        ...
        if result.success:
            _bitacora(
                f"Motor cmd completado: {command.id} dur={result.duration_ms:.0f}ms"
            )
        else:
    ...
    async def _execute_move_to(self, command: HighLevelCommand) -> ExecutionResult:
        ...
        if target_pose:
            ...
            pose = (
                Pose3D(**target_pose) if isinstance(target_pose, dict) else target_pose
            )
